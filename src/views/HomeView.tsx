...
export function HomeView({
  ...
  // 的回调里，等于在循环中调用 Hook——今天只因 kinds 是模块级常量、长度恒定
  // 才没出错，一旦它变成动态长度（按权限/媒体类型裁剪）就会破坏 Hook 顺序。
  const sampleByKind = useMemo(() => {
    const table = {} as Record<MediaKind, Artwork | null>;
    for (const item of kinds) {
      const works = getCollectionsByKind(item).flatMap((collection) => collection.works);
      table[item] = works.length > 0 ? works[Math.floor(Math.random() * works.length)] : null;
    }
    return table;
  }, []);
  ...
  return (
    <>
        ...
        <div className="orb-hero-inner">
          ...
          <div className="home-actions glass-capsule">
            <button className="button primary" onClick={() => chooseKind(kind)}>
              <Play size={16} />
              {t("开始准备", "Start preparing")}
            </button>
            <button className="button secondary" onClick={() => navigateTo("compare")}>
              <Users size={16} />
              {t("开始相遇", "Start encounter")}
            </button>
            {draft && (
              <button className="button quiet" onClick={resume}>
                <Play size={16} />
                {t("继续上次进度", "Resume")}
              </button>
            )}
          </div>
          <div className="orb-index" aria-hidden="true">
            <span>01</span>
            <i />
            <span>∞</span>
          </div>
      ...
      <section className="profile-overview">
        <div className="section-heading">
          <h2>{t("品味年轮", "Taste Rings")}</h2>
          <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
            {profile && (
              <button className="text-button" onClick={() => navigateTo("profile")}>
                {t("我的文化索引", "My Cultural Index")}
                <ArrowRight size={15} />
              </button>
            )}
            {profile && (
                ...
                {["row", "col"].map((m) => (
                  <button
                    ...
                    onClick={() => {
                      ...
                      try {
                        localStorage.setItem("art-rank:rings-layout", m);
                      } catch {}
                    }}
                    style={{
                      width: 24,
        ...
        {profile ? (
          <div
            style={{
              display: "flex",
              flexDirection: ringsLayout === "row" ? "column" : "row",
              gap: 20,
              flexWrap: "wrap",
            }}
          >
            {kinds
              .map((kind) => {
                const entries = profile.rankings
                  .map((r, i) => ({ ...r, _idx: i }))
                ...
                return (
                  <div
                    key={kind}
                    style={{
                      flex: ringsLayout === "row" ? "none" : "1 1 250px",
                      minWidth: ringsLayout === "row" ? "auto" : 200,
                    }}
                  >
                    <div
                      style={{
                    ...
                    >
                      {label(kind as MediaKind)}
                    </div>
                    <div
                      style={{
                        display: "flex",
                        flexDirection: ringsLayout === "row" ? "row" : "column",
                        gap: 10,
                        flexWrap: "wrap",
                      }}
                    >
                      {entries.map((entry) => {
                        ...
                        const isActive = editingRankIdx === idx;
                        return (
                          <div
                            key={`${entry.kind}-${entry.collectionTitle}-${idx}`}
                            className="coordinate"
                            style={{
                              position: "relative",
                              flex: ringsLayout === "row" ? "0 0 auto" : "none",
                            }}
                          >
                            <div
                              style={{
                            ...
                            >
                              ...
                              <div>
                                {isActive ? (
                                  ...
                                  <h3>{entry.collectionTitle}</h3>
                                )}
                                <small>
                                  {entry.items.length} {t("件作品", "works")}
                                </small>
                              </div>
                            </button>
                          </div>
          ...
          <div className="empty-profile">
            <div className="sample-covers" aria-hidden="true">
              {getCollectionsByKind("film")[1]
                ?.works.filter((work) => work.posterUrls?.length)
                .slice(0, 3)
                .map((work) => (
                  <Poster key={work.id} work={work} kind="film" />
                ))}
            </div>
            <div>
              <h3>{t("还没有完成的画像", "No completed profile yet")}</h3>
              <span>{t("电影 · 书籍 · 音乐 · 其他", "Films · Books · Music · Other")}</span>
            </div>
            {fileInput("own", t("导入我的画像", "Import my profile"), importProfile)}
          </div>
        )}
      </section>
    </>
