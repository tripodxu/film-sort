  ...
  return (
    <>
      ...
      {viewNotes[`profile:${peer.profileId}`] && (
        <div
          style={{
            ...
            padding: "12px 16px",
            borderRadius: 8,
            background: "color-mix(in srgb,var(--music) 6%,transparent)",
            border: "1px solid var(--line)",
          }}
        >
          <ExpandableNote
            text={viewNotes[`profile:${peer.profileId}`]}
            maxLength={30}
            onView={openNoteView ? (text) => openNoteView(peer.profileName, text) : undefined}
            style={{ margin: 0 }}
          />
        </div>
      )}
      ...
      {expiresAt && (
        <p
          className="mini-note"
          style={{ fontSize: 12, color: "var(--muted)", margin: "-8px 0 16px" }}
        >
          {t("此分享链接将于", "This share link expires on")}{" "}
          {new Date(expiresAt).toLocaleDateString()} {t("自动失效", "and will stop working then.")}
        </p>
      )}
      ...
      <div style={{ display: "flex", gap: 12, flexWrap: "wrap", margin: "24px 0" }}>
        ...
        <button
          ...
          onClick={() => {
            ...
            try {
              localStorage.setItem("art-rank:peer:v2", JSON.stringify(peer));
            } catch {}
            navigateTo("compare");
          }}
        >
      ...
      {!profile && (
        ...
        >
          ...
          >
            <Share2 size={15} />
            {t("如何创建自己的分享链接？", "How to create your own share link?")}
          </h3>
          <ol
            style={{
              fontSize: 13,
          ...
          >
            ...
            <li>
              {t(
                "在「我的文化索引」页面，点击「分享此榜单」或「复制比较链接」",
                "In 'My Culture Index', click 'Share this list' or 'Copy compare link'",
              )}
            </li>
            <li>
              {t(
                "将链接发给朋友，对方打开即可查看你的榜单",
                "Send the link to a friend — they can view your ranking when they open it",
              )}
            </li>
          </ol>
          <button
