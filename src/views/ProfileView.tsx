...
export function ProfileView({ profile, activeRanking, locale, t, label, format, setFormat, exportLayout, setExportLayout, exportProfile, share, shareUrl, qrUrl, profileName, setProfileName, namedProfile, persist, navigateTo, peer, editingRankIdx, setEditingRankIdx, editingRankTitle, setEditingRankTitle, renameRank, openCollection, shareSingleRanking, generateShareLink, setActiveKind, ranking, setRanking, collection, notes, openNoteModal, accountToken, publishToPlaza, reorderMode, reorderItems, startReorder, saveReorder, cancelReorder, moveItem, busy }: ProfileViewProps) {
  ...
  function closeOverflowMenu(event: MouseEvent) {
      ...
      <section>
        <div className="section-heading">
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            ...
            {!isReordering && <button className="text-button" onClick={() => { const works = activeRanking.items.map(item => ({ id: item.id, title: item.title, subtitle: item.subtitle, creator: item.creator, year: item.year, posterUrls: item.posterUrls })); openCollection({ id: `rerank-${activeRanking.profileId}`, kind: activeRanking.kind, source: "custom", title: activeRanking.collectionTitle, description: "", topN: activeRanking.items.length, works }); }} style={{ fontSize: 12, color: "var(--accent)" }}>{t("重新排序", "Re-rank")}</button>}
            <details className="overflow-menu">
              <summary aria-label={t("更多操作", "More actions")} title={t("更多操作", "More actions")}>⋯</summary>
              <div className="overflow-menu-panel">
                <button onClick={(e) => { closeOverflowMenu(e); setEditingRankIdx(profileRankIdx); setEditingRankTitle(activeRanking.collectionTitle); }}>{t("改名", "Rename")}</button>
                <button onClick={(e) => { closeOverflowMenu(e); void shareSingleRanking(activeRanking); }}>{t("比较链接", "Compare link")}</button>
                <button onClick={(e) => { closeOverflowMenu(e); void generateShareLink(activeRanking); }}>{t("分享链接", "Share link")}</button>
                {accountToken && <button onClick={(e) => { closeOverflowMenu(e); setShowPublishModal(true); }}>{t("发布到广场", "Publish to plaza")}</button>}
                {!isReordering && <button onClick={(e) => { closeOverflowMenu(e); startReorder(profileRankIdx); }}>{t("手动调整", "Manual order")}</button>}
              </div>
            </details>
          </div>
        </div>
          ...
          <ol className="ranking-list">
            {activeRanking.items.map((work) => (
              <li key={work.id}>
                ...
                <button className="icon-button" title={t("批注", "Note")} onClick={(e) => { e.stopPropagation(); openNoteModal(noteKey("work", activeRanking.kind, work.id), work.title, activeRanking.kind, work.posterUrls); }} style={{ width: 36, height: 36, marginLeft: "auto", color: hasNote(notes, noteKey("work", activeRanking.kind, work.id)) ? "var(--accent)" : "var(--muted)", opacity: hasNote(notes, noteKey("work", activeRanking.kind, work.id)) ? 1 : 0.4 }}><StickyNote size={14} /></button>
              </li>
            ))}
          </ol>
        )}
        ...
        {items.map((work, idx) => (
          ...
          >
            ...
            <div style={{ display: "flex", gap: 2, marginLeft: "auto", flexShrink: 0 }}>
              <button className="icon-button" title={t("上移", "Move up")} disabled={idx === 0} onClick={() => moveItem(idx, idx - 1)} style={{ width: 36, height: 36 }}><ArrowUp size={14} /></button>
              <button className="icon-button" title={t("下移", "Move down")} disabled={idx === items.length - 1} onClick={() => moveItem(idx, idx + 1)} style={{ width: 36, height: 36 }}><ArrowDown size={14} /></button>
              <button className="icon-button" title={t("批注", "Note")} onClick={(e) => { e.stopPropagation(); openNoteModal(noteKey("work", kind, work.id), work.title, kind, work.posterUrls); }} style={{ width: 36, height: 36, color: hasNote(notes, noteKey("work", kind, work.id)) ? "var(--accent)" : "var(--muted)", opacity: hasNote(notes, noteKey("work", kind, work.id)) ? 1 : 0.4 }}><StickyNote size={14} /></button>
            </div>
          </li>
        ))}
      </ol>
