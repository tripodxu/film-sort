...
export function ProfileView({
  ...
  return (
    <>
      {heading(
        t("我的文化索引", "MY CULTURE INDEX"),
        profile.profileName,
        `${profile.rankings.length} ${t("个领域", "media")} / ${profile.rankings.reduce((count, entry) => count + entry.items.length, 0)} ${t("件作品", "works")}`,
      )}
      <div
        style={{
          display: "flex",
      ...
      >
        <button
          className="rank-pill"
          title={t("批注此画像", "Annotate this profile")}
          onClick={() =>
            openNoteModal(noteKey("profile", profile.profileId), profile.profileName, "film")
          }
          style={
        ...
        >
          <Share2 size={13} />
          {busy ? t("生成中…", "Generating…") : t("分享画像", "Share profile")}
        </button>
        {accountToken && (
          <button
            className="rank-pill accent"
            title={t("将完整画像发布到广场", "Publish profile to plaza")}
            disabled={busy}
            onClick={() => {
          ...
          >
            <Globe size={13} />
            {t("发布画像", "Publish profile")}
          </button>
        )}
      </div>
      {format === "png" && (
        <div className="export-layout-switch">
          <span>{t("PNG 版式", "PNG layout")}</span>
          <div className="segmented">
            <button
              className={exportLayout === "editorial" ? "active" : ""}
              onClick={() => setExportLayout("editorial")}
            >
              {t("编辑", "Editorial")}
            </button>
            <button
              className={exportLayout === "podium" ? "active" : ""}
              onClick={() => setExportLayout("podium")}
            >
              {t("领奖台", "Podium")}
            </button>
            <button
              className={exportLayout === "collage" ? "active" : ""}
              onClick={() => setExportLayout("collage")}
            >
              {t("拼贴", "Collage")}
            </button>
            <button
              className={exportLayout === "filmstrip" ? "active" : ""}
              onClick={() => setExportLayout("filmstrip")}
            >
              {t("胶片", "Filmstrip")}
            </button>
            <button
              className={exportLayout === "minimal" ? "active" : ""}
              onClick={() => setExportLayout("minimal")}
            >
              {t("极简", "Minimal")}
            </button>
          </div>
      ...
      <div className="profile-layout">
        <section>
          <div className="section-heading">
            {isRenamingProfile ? (
              <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                <input
                  type="text"
                  value={editingRankTitle}
                  onChange={(e) => setEditingRankTitle(e.target.value)}
                  onKeyDown={(e) => {
                    if (e.key === "Enter") renameRank(profileRankIdx);
                    if (e.key === "Escape") setEditingRankIdx(null);
                  }}
                  style={{
                    fontSize: 18,
                    fontWeight: 600,
              ...
              <h2 style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0 }}>
                ...
                <button
                  className="rank-pill"
                  title={t("批注此榜单", "Annotate this list")}
                  onClick={() =>
                    openNoteModal(
                      noteKey("ranking", activeRanking.kind, activeRanking.collectionTitle),
                      activeRanking.collectionTitle,
                      activeRanking.kind,
                      [activeRanking.items[0]?.posterUrls?.[0] ?? ""],
                    )
                  }
                  style={{
                    minHeight: 28,
            ...
            <div className="rank-actions">
              <span className="rank-date">
                ...
                {new Date(activeRanking.createdAt).toLocaleDateString(
                  locale === "zh" ? "zh-CN" : "en-US",
                )}
              </span>
              {!isReordering && !editWorksOpen && (
                <button className="rank-pill" onClick={() => startReorder(profileRankIdx)}>
                  <SquarePen size={13} />
                  {t("手动调整", "Reorder")}
                </button>
              )}
                  ...
                  <>
                    ...
                    <div className="rank-menu" role="menu">
                      <button
                        ...
                        onClick={() => {
                          setMenuOpen(false);
                          setEditingRankIdx(profileRankIdx);
                          setEditingRankTitle(activeRanking.collectionTitle);
                        }}
                      >
                        <Pencil size={13} />
                        {t("改名", "Rename")}
                      ...
                      <button
                        ...
                        onClick={() => {
                          setMenuOpen(false);
                          const works = activeRanking.items.map((item) => ({
                            id: item.id,
                            title: item.title,
                          ...
                          openCollection({
                            id: `rerank-${activeRanking.profileId}`,
                            kind: activeRanking.kind,
                            source: "custom",
                            title: activeRanking.collectionTitle,
                            description: "",
                            topN: activeRanking.items.length,
                            works,
                          });
                        }}
                      >
                      ...
                      <button
                        ...
                        onClick={() => {
                          setMenuOpen(false);
                          void shareSingleRanking(activeRanking);
                        }}
                      >
                        <Link size={13} />
                        {t("比较链接", "Compare link")}
          ...
          {confirmDelete && !isReordering && !editWorksOpen && (
            <div className="rank-delete-bar">
              ...
              <span style={{ flex: 1 }}>
                {t(
                  `确定删除榜单「${activeRanking.collectionTitle}」？此操作不可撤销。`,
                  `Delete "${activeRanking.collectionTitle}"? This cannot be undone.`,
                )}
              </span>
              <button
                className="rank-pill danger-solid"
                onClick={() => {
                  setConfirmDelete(false);
                  deleteRank(profileRankIdx);
                  setNotice(t("榜单已删除。", "List deleted."));
                }}
              >
                {t("确认删除", "Delete")}
              </button>
              <button className="rank-pill" onClick={() => setConfirmDelete(false)}>
                {t("取消", "Cancel")}
              </button>
            </div>
          )}
          ...
          {plazaSyncPostId !== null && !editWorksOpen && (
            ...
            >
              <span>
                {t(
                  "该榜单已发布到广场，作品变更后可同步更新广场帖子。",
                  "This list is on the Plaza. Sync your changes to the post.",
                )}
              </span>
              <button
                className="text-button"
                disabled={busy}
                onClick={() => {
                  syncPlazaPost(plazaSyncPostId, profileRankIdx);
                  setPlazaSyncPostId(null);
                }}
                style={{ fontSize: 12, flexShrink: 0 }}
              >
                {t("同步到广场", "Sync to plaza")}
              </button>
              <button
                className="text-button"
                onClick={() => setPlazaSyncPostId(null)}
              ...
              {activeRanking.items.map((work) => (
                <li key={work.id}>
                  ...
                  <div
                    ...
                    onClick={() => openArtworkDetail(work, activeRanking.kind)}
                    role="button"
                    tabIndex={0}
                    onKeyDown={(e) => {
                      if (e.key === "Enter" || e.key === " ") {
                        e.preventDefault();
                        openArtworkDetail(work, activeRanking.kind);
                      }
                    }}
                  >
                  ...
                  <button
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      openNoteModal(
                        noteKey("work", activeRanking.kind, work.id),
                        work.title,
                        activeRanking.kind,
                        work.posterUrls,
                      );
                    ...
                    style={{
                      ...
                      height: 28,
                      marginLeft: "auto",
                      color: hasNote(notes, noteKey("work", activeRanking.kind, work.id))
                        ? "var(--accent)"
                        : "var(--muted)",
                      opacity: hasNote(notes, noteKey("work", activeRanking.kind, work.id))
                        ? 1
                        : 0.4,
                    }}
                  >
        ...
        <aside className="export-tools">
          ...
          <button className="button secondary" onClick={share}>
            <Share2 size={16} />
            {t("复制比较链接", "Copy comparison link")}
          </button>
          <button className="button secondary" onClick={() => openShareModal()}>
            <Link size={16} />
          ...
          {shareUrl && (
            <div className="share-output">
              <input
                aria-label={t("比较链接", "Comparison link")}
                readOnly
                value={shareUrl}
                onFocus={(event) => event.target.select()}
              />
              {qrUrl && <img src={qrUrl} alt={t("比较二维码", "Comparison QR code")} />}
            </div>
          )}
          <button className="button quiet" onClick={() => navigateTo("home")}>
            <Plus size={16} />
            {t("添加另一个维度", "Add another medium")}
          </button>
          <button className="button quiet" onClick={() => navigateTo("compare")}>
            <Users size={16} />
            {peer
              ? t("继续与好友比较", "Continue comparison")
              : t("与他人比较", "Compare with someone")}
          </button>
          {ranking?.completed && collection?.kind === activeRanking.kind && (
            <button
              className="button quiet"
              onClick={() => {
                setRanking(undoLastAction(ranking));
                navigateTo("sorting");
              }}
            >
              <Undo2 size={16} />
              {t("返回最后一次取舍", "Revisit last choice")}
            </button>
          )}
        </aside>
      </div>
      ...
      {showPublishModal && (
        <div className="modal-backdrop" onClick={() => setShowPublishModal(false)}>
          <section
            className="account-dialog publish-modal"
            role="dialog"
            aria-modal="true"
          ...
          >
            <div className="section-heading">
              ...
              <IconButton title={t("关闭", "Close")} onClick={() => setShowPublishModal(false)}>
                <X size={18} />
              </IconButton>
            </div>
            <p style={{ fontSize: 13, color: "var(--muted)", lineHeight: 1.7 }}>
              {publishTarget === "profile"
                ? t(
                    `将画像「${profile.profileName}」（${profile.rankings.length} 个榜单）发布到文化广场，其他用户可以看到、点赞、留言和使用其中的榜单排序。`,
                    `Publish profile "${profile.profileName}" (${profile.rankings.length} lists) to the Culture Plaza.`,
                  )
                : t(
                    `将「${activeRanking.collectionTitle}」发布到文化广场，其他用户可以看到、点赞、留言和使用你的榜单排序。`,
                    `Publish "${activeRanking.collectionTitle}" to the Culture Plaza. Others can see, like, comment, and sort with your ranking.`,
                  )}
            </p>
            <div className="publish-desc-label">
              <b>{t("榜单描述", "Description")}</b>
              <span>{publishDesc.length} / 500</span>
            </div>
            <textarea
              ...
              placeholder={t(
                "介绍一下这份榜单，让别人更懂你的品味…（可选）",
                "Introduce this list so others understand your taste… (optional)",
              )}
              rows={4}
              autoFocus
            />
            <div className="publish-preview">
              {t(
                "将随帖子公开展示，可在广场详情页看到。",
                "Shown publicly on your plaza post detail page.",
              )}
            </div>
            <div className="guide-modal-footer">
              ...
              >
                ...
                {publishBurst && (
                  <span className="publish-burst">
                    {Array.from({ length: 12 }).map((_, i) => {
                      const colors = ["var(--accent)", "#4ade80", "#818cf8", "#f472b6", "#facc15"];
                      return (
                        <span
                          key={i}
                          className="publish-particle"
          ...
          >
            ...
            <div style={{ display: "flex", gap: 2, marginLeft: "auto", flexShrink: 0 }}>
              ...
              <button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  openNoteModal(noteKey("work", kind, work.id), work.title, kind, work.posterUrls);
                }}
                style={{
                  width: 28,
                  height: 28,
                  color: hasNote(notes, noteKey("work", kind, work.id))
                    ? "var(--accent)"
                    : "var(--muted)",
                  opacity: hasNote(notes, noteKey("work", kind, work.id)) ? 1 : 0.4,
                }}
              >
                <StickyNote size={14} />
              </button>
...
function WorkEditor({
  ...
  const [results, setResults] = useState<
    Array<{ title: string; creator?: string; year?: string; rating?: string; poster?: string }>
  >([]);
  ...
  function parseManual() {
    manual.split(/\n+/).forEach((line) => {
      ...
      if (dashIdx > 0) {
        creator = s.slice(dashIdx + 3).trim();
  ...
  async function runSearch() {
    ...
    try {
      if (kind === "other") {
        const response = await fetch(`/api/other/list?key=${encodeURIComponent(key)}`, {
          signal: AbortSignal.timeout(20000),
        });
        const data = (await response.json()) as {
          data?: Array<{ title?: string; year?: number; subtitle?: string; poster_url?: string }>;
        };
        setResults(
          (data.data ?? [])
            .slice(0, 8)
      ...
      const response = await fetch(`/api/${apiType}/list?key=${encodeURIComponent(key)}&page=1`, {
        signal: AbortSignal.timeout(20000),
      });
      const data = (await response.json()) as {
        data?: Array<{
        ...
        (data.data ?? [])
          ...
          .map((item) => ({
            ...
            creator:
              item.author ||
              item.artist ||
              (Array.isArray(item.actors) ? item.actors[0] : undefined),
          }))
          .filter((item) => item.title),
      );
    } catch {
  ...
  return (
    <div>
      <div className="section-heading">
        <h2>{t("编辑作品", "Edit works")}</h2>
        <span>{t(`${items.length} 件`, `${items.length} works`)}</span>
      </div>
      <ol className="ranking-list">
        {items.map((work, idx) => (
          <li key={work.id}>
      ...
      >
        <div>
          <label style={{ fontSize: 12, color: "var(--muted)", display: "block", marginBottom: 6 }}>
            {t(
              "手动添加（每行一件，支持「标题 - 创作者（年份）」格式）",
              "Add manually (one per line: Title - Creator (Year))",
            )}
          </label>
          <textarea
            value={manual}
        ...
        </div>
        {
          <div>
            ...
            >
              {kind === "other"
                ? t("维基百科搜索添加", "Search Wikipedia to add")
                : t("豆瓣搜索添加", "Search Douban to add")}
            </label>
            <div style={{ display: "flex", gap: 8 }}>
              <input
                ...
                placeholder={t("输入关键词搜索", "Type keywords to search")}
                style={{ flex: 1 }}
              />
              <button
                className="button secondary"
                ...
                style={{ minHeight: 34, flexShrink: 0 }}
              >
                ...
                {results.map((item) => (
                  ...
                  >
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <strong>{item.title}</strong> {item.rating && <small>★{item.rating}</small>}
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        setResults((cur) => cur.filter((r) => r.title !== item.title));
                      }}
                      style={{ fontSize: 12, flexShrink: 0 }}
                    >
                      <Plus size={12} />
      ...
      <div style={{ display: "flex", gap: 8, marginTop: 16, justifyContent: "flex-end" }}>
        ...
        <button className="button primary" onClick={() => onSave(items)}>
          {t("保存", "Save")}
        </button>
      </div>
    </div>
