...
export function ProfileView({ profile, activeRanking, locale, t, label, format, setFormat, exportLayout, setExportLayout, exportProfile, share, shareUrl, qrUrl, profileName, setProfileName, namedProfile, persist, navigateTo, peer, editingRankIdx, setEditingRankIdx, editingRankTitle, setEditingRankTitle, renameRank, deleteRank, setNotice, openCollection, shareSingleRanking, openShareModal, setActiveKind, ranking, setRanking, collection, notes, openNoteModal, openArtworkDetail, accountToken, publishToPlaza, publishProfileToPlaza, updateRankingWorks, syncPlazaPost, reorderMode, reorderItems, startReorder, saveReorder, cancelReorder, moveItem, busy }: ProfileViewProps) {
  ...
  return <>
    ...
    {format === "png" && <div className="export-layout-switch"><span>{t("PNG 版式", "PNG layout")}</span><div className="segmented"><button className={exportLayout === "editorial" ? "active" : ""} onClick={() => setExportLayout("editorial")}>{t("编辑", "Editorial")}</button><button className={exportLayout === "podium" ? "active" : ""} onClick={() => setExportLayout("podium")}>{t("领奖台", "Podium")}</button><button className={exportLayout === "collage" ? "active" : ""} onClick={() => setExportLayout("collage")}>{t("拼贴", "Collage")}</button><button className={exportLayout === "filmstrip" ? "active" : ""} onClick={() => setExportLayout("filmstrip")}>{t("胶片", "Filmstrip")}</button><button className={exportLayout === "minimal" ? "active" : ""} onClick={() => setExportLayout("minimal")}>{t("极简", "Minimal")}</button></div></div>}
    <div className="profile-dimensions" role="tablist" aria-label={t("榜单切换", "Switch list")}>{profile.rankings.map((entry, idx) => { const key = `${entry.kind}-${idx}`; const active = activeRanking === entry; return <button className={`dim-chip medium-${entry.kind} ${active ? "active" : ""}`} key={key} role="tab" aria-selected={active} onClick={() => setActiveKind(key)}><Poster work={entry.items[0]} kind={entry.kind} /><div className="dim-chip-text"><span className="dim-chip-meta">{label(entry.kind)}<i>·</i>{entry.items.length} {t("件", "works")}</span><strong>{entry.collectionTitle}</strong></div></button>; })}</div>
