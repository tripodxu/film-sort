...
export function importCollection(kind: MediaKind, text: string): MediaCollection {
  ...
  if (text.trim().startsWith("[") || text.trim().startsWith("{")) {
    ...
    entries = Array.isArray(data)
      ? data
      : typeof data === "object" && data !== null && "works" in data && Array.isArray(data.works)
        ? data.works
        : [];
  } else
    entries = text
  ...
  for (const entry of entries) {
    ...
    if (year && (!Number.isInteger(year) || year < 1 || year > 2200))
      throw new Error("Invalid year");
    const creator =
      typeof item.creator === "string" ? item.creator.trim().slice(0, 160) : undefined;
    const identity = `${normalizeTitle(title)}|${year ?? ""}|${normalizeTitle(creator ?? "")}`;
    if (seen.has(identity)) continue;
    seen.add(identity);
    const urls = Array.isArray(item.posterUrls)
      ? item.posterUrls
      : item.poster_url
        ? [item.poster_url]
        : [];
    works.push({
      id: `custom-${works.length}`,
      title,
      year,
      creator,
