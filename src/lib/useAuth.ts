...
export function useAuth(deps: Omit<AuthDeps, 'setDraft'> & { setDraft?: (d: unknown) => void }) {
  ...
  const accountLoad = useCallback(async (autoApply: boolean) => {
    ...
    try {
      ...
      if (data.profile) {
        ...
        if (autoApply && !d.getProfile()) { d.persist(parsed); d.setNotice(d.t("已从云端恢复画像。", "Profile restored from cloud.")); }
        else if (!autoApply) { d.persist(parsed); d.setNotice(d.t("已从云端同步画像。", "Profile synced from cloud.")); }
      }
    ...
    } catch { if (!autoApply) d.setNotice(d.t("读取失败，请重新登录。", "Failed to load. Please sign in again.")); }
  ...
  const accountSave = useCallback(async () => {
    const d = depsRef.current;
    const token = stored("art-rank:account-token") ?? "";
    if (!token) { d.setNotice(d.t("请先登录。", "Please sign in first.")); return; }
    const next = d.namedProfile(); if (!next) return;
    setBusy(true);
    try {
      const hasAnyNotes = Object.keys(d.getNotes()).length > 0;
      const response = await fetch("/api/account/profile", { method: "PUT", headers: { "content-type": "application/json", authorization: `Bearer ${token}` }, body: JSON.stringify({ profile: next, ...(hasAnyNotes ? { notes: d.getNotes() } : {}) }) });
      if (!response.ok) throw new Error();
      d.persist(next); setSyncStatus("saved"); d.setNotice(d.t("画像已保存到云端。", "Profile saved to cloud."));
    } catch { d.setNotice(d.t("同步失败，本地画像仍然保留。", "Sync failed. Your local profile is still available.")); }
    finally { setBusy(false); }
  }, []);

  const accountAuth = useCallback(async (mode: "login" | "register") => {
    const d = depsRef.current;
    setAuthError(""); setBusy(true);
    try {
      if (mode === "register" && !authNickname.trim()) { setAuthError(d.t("请填写昵称", "Please enter a nickname")); setBusy(false); return; }
      ...
      if (!response.ok || !data.token) { setAuthError(data.msg ?? data.error ?? d.t("操作失败", "Failed")); return; }
      ...
      setTimeout(async () => {
        try {
          ...
          if (!r.ok) return;
          const dr = await r.json() as { nickname?: string; profile: unknown; notes?: Record<string, string> };
          ...
          if (dr.profile) {
            ...
            if (d.getProfile()) { setCloudConflict(cloudParsed); }
            else { d.persist(cloudParsed); d.setNotice(d.t("已从云端恢复画像和批注。", "Profile and notes restored from cloud.")); }
          }
        } catch {}
      }, 100);
    } catch { setAuthError(d.t("网络错误", "Network error")); }
    finally { setBusy(false); }
  ...
  const saveNickname = useCallback(async () => {
    ...
    setBusy(true);
    try {
      const response = await fetch("/api/account/nickname", { method: "PUT", headers: { "content-type": "application/json", authorization: `Bearer ${token}` }, body: JSON.stringify({ nickname: name }) });
      if (response.ok) { setAccountNickname(name); setNeedNickname(false); setEditingNickname(false); setAuthNickname(""); setEditNicknameValue(""); d.setNotice(d.t("昵称已更新！", "Nickname updated!")); }
    } catch {}
    finally { setBusy(false); }
  ...
  useEffect(() => {
    const flush = () => { void syncProfile(true); };
    const online = () => { void syncProfile(false); };
  ...
  return {
    // State
    accountOpen, setAccountOpen, accountEmail, setAccountEmail,
    accountNickname, setAccountNickname, accountToken, setAccountToken,
    authMode, setAuthMode, authEmail, setAuthEmail,
    authPassword, setAuthPassword, authNickname, setAuthNickname,
    authError, setAuthError, needNickname, setNeedNickname,
    editingNickname, setEditingNickname, editNicknameValue, setEditNicknameValue,
    syncStatus, setSyncStatus, cloudConflict, setCloudConflict,
    busy, setBusy,
    // Functions
