...
export default function App() {
  ...
  if (view === "home") content = <>
    ...
    <div className="setup-layout"><section className="settings"><label htmlFor="top-n">Top N <strong>{Math.min(topN, selected.length)}</strong></label><input id="top-n" type="range" min={1} max={Math.max(1, selected.length)} value={Math.min(topN, selected.length) || 1} onChange={(event) => setTopN(Number(event.target.value))} /><label htmlFor="seed">{t("顺序口令（可选）", "Order seed (optional)")}</label><input id="seed" maxLength={80} value={seed} onChange={(event) => setSeed(event.target.value)} /><label htmlFor="collection-name">{t("榜单名称", "Collection name")}</label><input id="collection-name" maxLength={160} value={collection.title} onChange={(event) => setCollection({ ...collection, title: event.target.value })} /><button className="button primary" disabled={selected.length < 2 || !collection.title.trim()} onClick={startRanking}><Play size={16} />{t("开始 1v1 取舍", "Start 1v1 ranking")}</button></section><section className="candidate-list"><div className="section-heading"><h2>{t("已看 / 已读 / 已听", "Experienced works")}</h2><div style={{ display: "flex", alignItems: "center", gap: 8 }}><div style={{ display: "flex", gap: 4 }}>{[2,3,4].map(n => <button key={n} onClick={() => changeCols(n)} style={{ width: 28, height: 28, borderRadius: 6, border: colCount === n ? "1px solid var(--accent)" : "1px solid var(--line)", background: colCount === n ? "rgba(216,248,106,.1)" : "transparent", color: colCount === n ? "var(--accent)" : "var(--muted)", fontSize: 11, fontWeight: 600, cursor: "pointer" }}>{n}</button>)}</div><label className="check-all"><input type="checkbox" checked={selected.length === collection.works.length} onChange={(event) => setSelected(event.target.checked ? collection.works.map((work) => work.id) : [])} />{selected.length} / {collection.works.length}</label></div></div><div className="candidate-scroll" style={{ gridTemplateColumns: `repeat(${colCount}, minmax(0, 1fr))` }}>{collection.works.map((work) => <label className="candidate-row" key={work.id}><input type="checkbox" checked={selected.includes(work.id)} onChange={(event) => setSelected(event.target.checked ? [...selected, work.id] : selected.filter((id) => id !== work.id))} /><Poster work={work} kind={kind} /><div><strong>{work.title}</strong><small>{work.creator} {work.year}</small></div></label>)}</div></section></div>
  </>;
