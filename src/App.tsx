...
export default function App() {
  ...
  const [noteViewModal, setNoteViewModal] = useState<{ title: string; text: string; posterUrls?: readonly string[] } | null>(null);
  ...
  function persist(next: ArtisticProfile) {
    ...
    try {
      ...
      setNotice(t("浏览器无法保存，请及时导出画像。", "Browser storage is unavailable. Export your profile to keep it."));
    }
  }
  ...
  useEffect(() => {
    ...
    if (payload) {
      if (/^[0-9a-f]{8,12}$/i.test(payload)) {
        ...
        fetch(`/api/share/${payload}`).then(async (r) => { if (!r.ok) throw new Error(); const d = await r.json() as { profile: unknown; notes?: Record<string, string> }; acceptPeer(parseProfile(d.profile)); if (d.notes && typeof d.notes === "object") setPeerNotes(d.notes); }).catch(() => setNotice(t("比较链接无效或已过期。", "Compare link is invalid or expired.")));
      } else {
      ...
      if (code) {
        if (/^[0-9a-f]{8,12}$/i.test(code)) {
          ...
          fetch(`/api/share/${code}`).then(async (r) => {
            ...
            const d = await r.json() as { profile: unknown; notes?: Record<string, string>; expires_at?: string };
  ...
  function updateRankingWorks(rankingIdx: number, works: RankedArtwork[]) {
    ...
    rankings[rankingIdx] = { ...rankings[rankingIdx], items: works.map((w, i) => ({ ...w, rank: i + 1 })) };
  ...
  async function syncPlazaPost(postId: number, rankingIdx: number) {
    ...
    setBusy(true);
    try {
      const response = await fetch(`/api/plaza/posts/${postId}`, { method: "PUT", headers: { "content-type": "application/json", authorization: `Bearer ${accountToken}` }, body: JSON.stringify({ items: ranking.items, item_count: ranking.items.length, edits: [{ action: "sync", detail: t("从文化索引同步榜单内容", "Synced from My Culture Index") }] }) });
      if (response.ok) setNotice(t("广场帖子已同步更新。", "Plaza post synced."));
      else setNotice(t("同步失败，请稍后重试。", "Sync failed. Please retry."));
    } catch { setNotice(t("同步失败，请稍后重试。", "Sync failed. Please retry.")); }
  ...
  async function shareSingleRanking(ranking: RankingExport) {
    ...
    try {
      ...
      const link = data.compareUrl ?? (data.url ? data.url.replace(/\/share\/([0-9a-f]{8,12})$/, "/encounter?payload=$1") : undefined);
      if (response.ok && link) {
  ...
  function openShareModal(ranking?: RankingExport) { setShareModal({ ranking }); }
  async function generateShareLink(ranking?: RankingExport, expiresDays: number = 30) {
    ...
    try {
      const response = await fetch("/api/share", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(shareData) });
      const data = await response.json() as { url?: string; expires_days?: number; error?: string };
      if (response.ok && data.url) {
        const days = data.expires_days ?? expiresDays;
        try { await navigator.clipboard.writeText(data.url); setNotice(t(`分享链接已复制，${days} 天后失效。`, `Share link copied. Expires in ${days} days.`)); }
  ...
  async function importPeerFromUrl() {
    ...
    try {
      ...
      if (shareMatch) {
        const r = await fetch(`/api/share/${shareMatch[1]}`); if (!r.ok) throw new Error();
  ...
  else if (view === "source") content = <SourceView setAccountOpen={setAccountOpen} kind={kind} t={t} label={label} source={source} setSource={setSource} setNotice={setNotice} search={search} setSearch={setSearch} colCount={colCount} changeCols={changeCols} collections={collections} openCollection={openCollection} customItem={customItem} setCustomItem={setCustomItem} customWorks={customWorks} customDeselected={customDeselected} setCustomDeselected={setCustomDeselected} importProgress={importProgress} searchWorks={searchWorks} addCustomWork={addCustomWork} removeCustomWork={removeCustomWork} clearCustomWorks={clearCustomWorks} loadCustomWorks={loadCustomWorks} saveCustomWorks={saveCustomWorks} accountToken={accountToken} importDoulist={importDoulist} importNeteasePlaylist={importNeteasePlaylist} customText={customText} setCustomText={setCustomText} importCollection={importCollection} saveCollectionCloud={saveCollectionCloud} cloudCollections={cloudCollections} loadCloudCollections={loadCloudCollections} deleteCloudCollection={deleteCloudCollection} doubanLimit={doubanLimit} setDoubanLimit={setDoubanLimit} busy={busy} loadDouban={loadDouban} />;
  else if (view === "setup" && collection) content = <SetupView collection={collection} kind={kind} t={t} label={label} selected={selected} setSelected={setSelected} topN={topN} setTopN={setTopN} seed={seed} setSeed={setSeed} setCollection={setCollection} startRanking={startRanking} saveWithoutSorting={saveWithoutSorting} />;
  ...
  else if (view === "plazaPost") content = <PlazaPostView postId={plazaPostId} t={t} label={label} navigateTo={navigateTo} accountToken={accountToken} accountNickname={accountNickname} openCollection={openCollection} profile={profile} setNotice={setNotice} setPeer={setPeer} notes={notes} openArtworkDetail={openArtworkDetail} openNoteView={openNoteView} />;
  ...
  return <div className="app-shell"><header className="topbar"><button className="wordmark" onClick={() => navigateTo("home")}>ART<span>/</span>RANK</button><nav aria-label={t("主导航", "Main navigation")}><button className={view === "home" || view === "source" || view === "setup" || view === "sorting" ? "active" : ""} onClick={() => navigateTo("home")}>{t("清单", "Catalog")}</button><button className={view === "compare" ? "active" : ""} onClick={() => navigateTo("compare")}>{t("相遇", "Encounter")}</button><button className={view === "plaza" || view === "plazaPost" ? "active" : ""} onClick={() => navigateTo("plaza")}>{t("广场", "Plaza")}</button>{profile && <button className={view === "profile" ? "active" : ""} onClick={() => navigateTo("profile")}>{t("我的文化索引", "My Index")}</button>}</nav><div className="header-tools"><SettingsMenu zh={locale === "zh"} cap={importCap} onCap={setImportCap} /><ThemeSwitcher zh={locale === "zh"} /><IconButton title={locale === "zh" ? "English" : "中文"} onClick={() => { const next = locale === "zh" ? "en" : "zh"; setLocale(next); const url = new URL(location.href); url.searchParams.set("lang", next); history.replaceState(null, "", url); }}><Languages size={18} /></IconButton><a className="icon-button" href="https://github.com/tripodxu/film-sort" target="_blank" rel="noopener noreferrer" title="GitHub"><Github size={18} /><span className="tooltip" role="tooltip">GitHub</span></a><IconButton title={accountEmail ? t("同步 / 退出", "Sync / Sign out") : t("登录 / 同步", "Sign in / Sync")} onClick={() => setAccountOpen(true)}><UserRound size={18} />{accountToken && <span className="sync-dot" data-status={syncStatus} style={{ position: "absolute", top: 4, right: 4, width: 7, height: 7, borderRadius: "50%", border: "1.5px solid var(--bg)", zIndex: 1 }} />}</IconButton></div></header>
    ...
    {detailWork && <ArtworkDetail detail={detailWork} label={label} t={t} onClose={() => setDetailWork(null)} />}
    {shareModal && <div className="modal-backdrop" onClick={() => setShareModal(null)}><section className="account-dialog" role="dialog" aria-modal="true" aria-labelledby="share-expiry-heading" onClick={(event) => event.stopPropagation()} onKeyDown={(event) => { if (event.key === "Escape") setShareModal(null); }}><div className="section-heading"><div><span className="eyebrow">SHARE</span><h2 id="share-expiry-heading">{t("分享链接有效期", "Share link expiry")}</h2></div><IconButton title={t("关闭", "Close")} onClick={() => setShareModal(null)}><X size={18} /></IconButton></div><p style={{ fontSize: 13, color: "var(--muted)", lineHeight: 1.7, margin: 0 }}>{t("选择链接有效期，到期后他人将无法再打开该链接。生成后链接自动复制。", "Pick how long the link stays valid — it stops working after expiry. The link is copied automatically.")}</p><div style={{ display: "flex", flexDirection: "column", gap: 8 }}>{[7, 30, 90, 365].map((days) => <button key={days} className="button secondary" disabled={busy} style={{ justifyContent: "space-between" }} onClick={() => { const picked = shareModal; setShareModal(null); void generateShareLink(picked?.ranking, days); }}><span>{days === 365 ? t("一年", "1 year") : `${days} ${t("天", "days")}`}</span><small>{new Date(Date.now() + days * 86400000).toLocaleDateString(locale === "zh" ? "zh-CN" : "en-US")}</small></button>)}</div></section></div>}
    {noteModal && <div className="modal-backdrop note-backdrop" onClick={() => setNoteModal(null)}><section className="note-reader" role="dialog" aria-modal="true" aria-labelledby="note-heading" onClick={(event) => event.stopPropagation()} onKeyDown={(event) => { if (event.key === "Escape") setNoteModal(null); }}><div className="note-reader-bg" style={{ backgroundImage: noteModal.posterUrls?.[0] ? `url(/api/image?url=${encodeURIComponent(noteModal.posterUrls[0])})` : undefined }} /><div className="note-reader-header"><span className="note-reader-eyebrow">NOTE</span><h2 id="note-heading">{noteModal.title}</h2></div><textarea className="note-reader-body" value={noteDraft} onChange={(e) => setNoteDraft(e.target.value)} placeholder={t("写下你对这部作品的看法、感想、回忆…", "Write your thoughts, feelings, memories about this work...")} autoFocus /><div className="note-reader-footer"><button className="note-footer-btn" onClick={() => { const next = setNote(notes, noteModal.key, ""); setNotes(next); writeNotes(next); setNoteDraft(""); setNotice(t("批注已清除。", "Note cleared.")); }}>{t("清除", "Clear")}</button>{noteDraft.length > 0 && <span className="note-char-count">{noteDraft.length} {t("字", "chars")}</span>}<div style={{ flex: 1 }} /><button className="note-footer-btn" onClick={() => setNoteModal(null)}>{t("取消", "Cancel")}</button><button className="note-footer-btn note-footer-primary" onClick={saveNote}>{t("保存", "Save")}</button></div></section></div>}
    {noteViewModal && <div className="modal-backdrop note-backdrop" onClick={() => setNoteViewModal(null)}><section className="note-reader" role="dialog" aria-modal="true" onClick={(event) => event.stopPropagation()} onKeyDown={(event) => { if (event.key === "Escape") setNoteViewModal(null); }}><div className="note-reader-bg" style={{ backgroundImage: noteViewModal.posterUrls?.[0] ? `url(/api/image?url=${encodeURIComponent(noteViewModal.posterUrls[0])})` : undefined }} /><div className="note-reader-header"><span className="note-reader-eyebrow">NOTE</span><h2>{noteViewModal.title}</h2></div><div className="note-reader-body note-reader-view"><p>{noteViewModal.text}</p></div><div className="note-reader-footer"><div style={{ flex: 1 }} /><button className="note-footer-btn note-footer-primary" onClick={() => setNoteViewModal(null)}>{t("关闭", "Close")}</button></div></section></div>}
    ...
    {compareRankDetail && <div className="modal-backdrop" onClick={() => setCompareRankDetail(null)}><section className="rank-detail-dialog" role="dialog" aria-modal="true" onClick={(event) => event.stopPropagation()} onKeyDown={(event) => { if (event.key === "Escape") setCompareRankDetail(null); }}>{compareRankDetail.ranking ? <RankingDetail kind={compareRankDetail.ranking.kind} collectionTitle={compareRankDetail.collectionTitle} items={compareRankDetail.ranking.items} notes={compareRankDetail.side === "peer" ? peerNotes : notes} kindLabel={label} eyebrow={`${compareRankDetail.side === "own" ? t("我的索引", "MY INDEX") : t("对方索引", "THEIR INDEX")} / ${label(compareRankDetail.ranking.kind)}`} headerExtra={<IconButton title={t("关闭", "Close")} onClick={() => setCompareRankDetail(null)}><X size={18} /></IconButton>} onNoteView={(title, text, posterUrls) => { openNoteView(title, text, posterUrls); }} onArtworkClick={(work, kind) => { void openArtworkDetail(work, kind); }} highlightId={compareRankDetail.highlightId} /> : <div className="section-heading"><h2>{compareRankDetail.collectionTitle}</h2><IconButton title={t("关闭", "Close")} onClick={() => setCompareRankDetail(null)}><X size={18} /></IconButton></div>}{!compareRankDetail.ranking && <p className="empty-state">{t("无法找到该榜单数据", "Ranking data not found")}</p>}</section></div>}
    {accountOpen && <div className="modal-backdrop" onClick={() => setAccountOpen(false)}><section className="account-dialog" role="dialog" aria-modal="true" aria-labelledby="account-heading" onClick={(event) => event.stopPropagation()} onKeyDown={(event) => { if (event.key === "Escape") setAccountOpen(false); }}><div className="section-heading"><h2 id="account-heading">{t("账号与同步", "Account & sync")}</h2><IconButton title={t("关闭", "Close")} onClick={() => setAccountOpen(false)}><X size={18} /></IconButton></div>
      {accountEmail && needNickname ? <>
        ...
        <p style={{ marginBottom: 16, fontSize: 12, color: "var(--muted)", display: "flex", alignItems: "center", gap: 6 }}>{accountEmail}<span title={syncStatus === "saving" ? t("同步中", "Syncing") : syncStatus === "saved" ? t("已同步", "Synced") : syncStatus === "error" ? t("同步失败", "Sync failed") : t("待机", "Idle")} className="sync-dot" data-status={syncStatus} style={{ display: "inline-block", width: 8, height: 8, borderRadius: "50%", flexShrink: 0 }} /></p>
