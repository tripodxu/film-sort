...
export default function App() {
  ...
  if (view === "home") content = <>
    ...
    {heading(`${label(collection.kind)} / ${t("准备", "PREPARE")}`, collection.title, t("选择参与比较的作品，设定你想留下的 Top N。", "Choose the works and set the Top N you want to keep."))}
    <div className="setup-layout"><section className="settings"><label htmlFor="top-n">Top N <strong>{topN}</strong></label><div style={{ display: "flex", gap: 10, alignItems: "center" }}><input id="top-n" type="range" min={1} max={Math.max(1, selected.length)} value={Math.min(topN, selected.length) || 1} onChange={(event) => setTopN(Number(event.target.value))} style={{ flex: 1 }} /><input type="number" min={1} max={selected.length || 1} value={topN} onChange={(event) => { const v = Number(event.target.value); if (v >= 1) setTopN(v); }} style={{ width: 64, minHeight: 36, textAlign: "center", padding: "4px 6px" }} /></div><label htmlFor="seed">{t("顺序口令（可选）", "Order seed (optional)")}</label><input id="seed" maxLength={80} value={seed} onChange={(event) => setSeed(event.target.value)} /><label htmlFor="collection-name">{t("榜单名称", "Collection name")}</label><input id="collection-name" maxLength={160} value={collection.title} onChange={(event) => setCollection({ ...collection, title: event.target.value })} /><button className="button primary" disabled={selected.length < 2 || !collection.title.trim()} onClick={startRanking}><Play size={16} />{t("开始相遇", "Start encounter")}</button></section><section className="candidate-list"><div className="section-heading"><h2>{t("已看 / 已读 / 已听", "Experienced works")}</h2><label className="check-all"><input type="checkbox" checked={selected.length === collection.works.length} onChange={(event) => setSelected(event.target.checked ? collection.works.map((work) => work.id) : [])} />{selected.length} / {collection.works.length}</label></div><div className="candidate-scroll">{collection.works.map((work) => <label className="candidate-row" key={work.id}><input type="checkbox" checked={selected.includes(work.id)} onChange={(event) => setSelected(event.target.checked ? [...selected, work.id] : selected.filter((id) => id !== work.id))} /><Poster work={work} kind={kind} /><div><strong>{work.title}</strong><small>{work.creator} {work.year}</small></div></label>)}</div></section></div>
  </>;
  else if (view === "sorting" && collection && ranking && comparison && progress) content = <>
    <div className="duel-heading"><div><span className="eyebrow">{label(kind)} / {t("相遇", "ENCOUNTER")} TOP {ranking.topN}</span><h1>{collection.title}</h1></div><div className="comparison-count"><strong>{progress.comparisonCount}</strong><span>{t("次取舍", "choices")}</span></div></div>
...
<div className="profile-layout"><section><div className="section-heading">{isRenamingProfile ? <div style={{ display: "flex", gap: 6, alignItems: "center" }}><input type="text" value={editingRankTitle} onChange={(e) => setEditingRankTitle(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") renameRank(profileRankIdx); if (e.key === "Escape") setEditingRankIdx(null); }} style={{ fontSize: 18, fontWeight: 600, padding: "4px 8px", minHeight: "auto", flex: 1 }} autoFocus /><button className="text-button" onClick={() => renameRank(profileRankIdx)} style={{ color: "var(--accent)", fontSize: 14 }}>✓</button></div> : <h2>{activeRanking.collectionTitle}</h2>}<div style={{ display: "flex", gap: 8, alignItems: "center" }}><span>{new Date(activeRanking.createdAt).toLocaleDateString(locale === "zh" ? "zh-CN" : "en-US")}</span><button className="text-button" onClick={() => { setEditingRankIdx(profileRankIdx); setEditingRankTitle(activeRanking.collectionTitle); }} style={{ fontSize: 12, color: "var(--muted)" }}>{t("改名", "Rename")}</button><button className="text-button" onClick={() => { const works = activeRanking.items.map(item => ({ id: item.id, title: item.title, subtitle: item.subtitle, creator: item.creator, year: item.year, posterUrls: item.posterUrls })); openCollection({ id: `rerank-${activeRanking.profileId}`, kind: activeRanking.kind, source: "custom", title: activeRanking.collectionTitle, description: "", topN: activeRanking.items.length, works }); }} style={{ fontSize: 12, color: "var(--accent)" }}>{t("重新排序", "Re-rank")}</button><button className="text-button" onClick={() => void shareSingleRanking(activeRanking)} style={{ fontSize: 12, color: "var(--accent)" }}>{t("分享此榜单", "Share this list")}</button></div></div><ol className="ranking-list">{activeRanking.items.map((work) => <li key={work.id}><span className="row-number">{String(work.rank).padStart(2, "0")}</span><Poster work={work} kind={activeRanking.kind} /><div><strong>{work.title}</strong><small>{work.creator} {work.year}</small></div>{work.rank <= 3 && <span style={{ fontSize: work.rank === 1 ? 20 : 16 }}>{work.rank === 1 ? "🥇" : work.rank === 2 ? "🥈" : "🥉"}</span>}</li>)}</ol></section><aside className="export-tools"><label htmlFor="profile-name">{t("画像名称", "Profile name")}</label><input id="profile-name" value={profileName} maxLength={80} onChange={(event) => { setProfileName(event.target.value); setShareUrl(""); setQrUrl(""); }} onBlur={() => { const next = namedProfile(); if (next) persist(next); }} /><label htmlFor="export-format">{t("导出格式", "Export format")}</label><select id="export-format" value={format} onChange={(event) => setFormat(event.target.value as typeof format)}>{["json", "png", "txt", "csv", "md"].map((item) => <option key={item} value={item}>{item.toUpperCase()}</option>)}</select><button className="button primary" onClick={exportProfile}><Download size={16} />{t("导出全部维度", "Export all media")}</button><button className="button secondary" onClick={share}><Share2 size={16} />{t("复制比较链接", "Copy comparison link")}</button>{shareUrl && <div className="share-output"><input aria-label={t("比较链接", "Comparison link")} readOnly value={shareUrl} onFocus={(event) => event.target.select()} />{qrUrl && <img src={qrUrl} alt={t("比较二维码", "Comparison QR code")} />}</div>}<button className="button quiet" onClick={() => navigateTo("home")}><Plus size={16} />{t("添加另一个维度", "Add another medium")}</button><button className="button quiet" onClick={() => navigateTo("compare")}><Users size={16} />{peer ? t("继续与好友比较", "Continue comparison") : t("与他人比较", "Compare with someone")}</button>{ranking?.completed && collection?.kind === activeRanking.kind && <button className="button quiet" onClick={() => { setRanking(undoLastAction(ranking)); navigateTo("sorting"); }}><Undo2 size={16} />{t("返回最后一次取舍", "Revisit last choice")}</button>}</aside></div>
    ...
    const result = (() => {
      if (compareMode === "manual") {
        ...
        if (selectedOwn.length === 1 && selectedPeer.length === 1) return compareRankings(selectedOwn[0], selectedPeer[0]);
    ...
    content = <>{heading(t("相遇", "ENCOUNTER"), t("看看你们的选择在哪里重合", "See where your choices meet"), peer ? t("两份索引的交集与分歧", "Overlap and divergence of two indexes") : t("分享你的索引，或导入对方的来比较。", "Share your index or import theirs to compare."))}{peer && <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 12, gap: 8 }}><button className="text-button" onClick={() => { setPeer(null); navigateTo("compare"); setAiInsight(""); setCompareActiveKind("film"); setCompareMode("auto"); setManualOwnSelections(new Set()); setManualPeerSelections(new Set()); }} style={{ fontSize: 12, color: "var(--muted)" }}><X size={14} style={{ verticalAlign: "middle", marginRight: 4 }} />{t("退出比较", "Exit comparison")}</button></div>}{peer && profile && <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 16 }}><div style={{ display: "flex", alignItems: "center", gap: 8 }}><span style={{ fontSize: 11, color: "var(--muted)" }}>{t("比较模式", "Mode")}:</span><div className="segmented" style={{ marginBottom: 0 }}><button role="tab" aria-selected={compareMode === "auto"} className={compareMode === "auto" ? "active" : ""} onClick={() => setCompareMode("auto")}>{t("自动合并", "Auto merge")}</button><button role="tab" aria-selected={compareMode === "manual"} className={compareMode === "manual" ? "active" : ""} onClick={() => setCompareMode("manual")}>{t("指定榜单", "Pick lists")}</button></div>{compareMode === "manual" && (selectedOwn.length === 0 || selectedPeer.length === 0) && <div className="empty-state" style={{ marginBottom: 12 }}><p style={{ fontSize: 12, color: "var(--muted)" }}>{t("请至少各选一个榜单", "Select at least one list from each side")}</p></div>}</div></div>}<div className="comparison-inputs"><section><span className="eyebrow">01 / {t("我的索引", "MY INDEX")}</span><h2>{profile?.profileName ?? t("尚未创建", "Not created")}</h2>{profile ? <div style={{ marginTop: 8, display: "flex", flexDirection: "column", gap: 4 }}>{profile.rankings.map((entry, idx) => { const kindIdx = profile.rankings.filter((r) => r.kind === entry.kind).indexOf(entry); const showCheck = compareMode === "manual" && entry.kind === compareKind; return <div key={idx} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, opacity: showCheck && !manualOwnSelections.has(kindIdx) ? 0.4 : 1 }}>{showCheck && <input type="checkbox" checked={manualOwnSelections.has(kindIdx)} onChange={() => { const next = new Set(manualOwnSelections); next.has(kindIdx) ? next.delete(kindIdx) : next.add(kindIdx); setManualOwnSelections(next); }} style={{ flexShrink: 0 }} />}<span style={{ color: "var(--accent)", fontSize: 11, fontWeight: 600, width: 28, flexShrink: 0 }}>{label(entry.kind)}</span><Poster work={entry.items[0]} kind={entry.kind} /><span style={{ flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{entry.collectionTitle}</span><span style={{ color: "var(--muted)", fontSize: 11, flexShrink: 0 }}>TOP {entry.items.length}</span></div>; })}</div> : fileInput("own", t("导入我的索引", "Import my index"))}</section><section><span className="eyebrow">02 / {t("对方索引", "THEIR INDEX")}</span><h2>{peer?.profileName ?? t("等待导入", "Awaiting import")}</h2>{peer ? <div style={{ marginTop: 8, display: "flex", flexDirection: "column", gap: 4 }}>{peer.rankings.map((entry, idx) => { const kindIdx = peer.rankings.filter((r) => r.kind === entry.kind).indexOf(entry); const showCheck = compareMode === "manual" && entry.kind === compareKind; return <div key={idx} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, opacity: showCheck && !manualPeerSelections.has(kindIdx) ? 0.4 : 1 }}>{showCheck && <input type="checkbox" checked={manualPeerSelections.has(kindIdx)} onChange={() => { const next = new Set(manualPeerSelections); next.has(kindIdx) ? next.delete(kindIdx) : next.add(kindIdx); setManualPeerSelections(next); }} style={{ flexShrink: 0 }} />}<span style={{ color: "var(--accent)", fontSize: 11, fontWeight: 600, width: 28, flexShrink: 0 }}>{label(entry.kind)}</span><Poster work={entry.items[0]} kind={entry.kind} /><span style={{ flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{entry.collectionTitle}</span><span style={{ color: "var(--muted)", fontSize: 11, flexShrink: 0 }}>TOP {entry.items.length}</span></div>; })}</div> : <div style={{ marginTop: 8, display: "flex", flexDirection: "column", gap: 8 }}><div style={{ display: "flex", gap: 6 }}><input type="url" placeholder={t("粘贴比较链接或 JSON 地址…", "Paste compare link or JSON URL…")} value={peerUrl} onChange={(e) => setPeerUrl(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") void importPeerFromUrl(); }} style={{ flex: 1, minHeight: 38, fontSize: 13 }} /><button className="button secondary" disabled={!peerUrl.trim() || peerUrlBusy} onClick={() => void importPeerFromUrl()} style={{ minHeight: 38, padding: "0 12px" }}>{peerUrlBusy ? "…" : t("导入", "Import")}</button></div>{fileInput("peer", t("或上传 JSON 文件", "or upload JSON"))}</div>}</section></div>
      {!peer && profile && <div className="empty-state"><p style={{ marginBottom: 12 }}>{t("分享你的索引链接，或导入对方的来比较。", "Share your index link, or import theirs to compare.")}</p><div style={{ display: "flex", gap: 8, flexWrap: "wrap", justifyContent: "center" }}><button className="button secondary" onClick={() => { setFormat("json"); void exportProfile(); }}>{t("导出索引文件", "Export index file")}</button><button className="button secondary" disabled={busy} onClick={() => { const next = namedProfile(); if (!next) return; setBusy(true); fetch("/api/share", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ profile: next }) }).then(async (r) => { const d = await r.json() as { url?: string; error?: string }; if (r.ok && d.url) { navigator.clipboard?.writeText(d.url); setNotice(t("比较链接已复制到剪贴板，发给对方即可比较。", "Compare link copied. Share it to compare.")); } else { setNotice(t("生成链接失败，请导出文件。", "Failed to create link. Export file instead.")); } }).catch(() => setNotice(t("生成链接失败，请导出文件。", "Failed to create link. Export file instead."))).finally(() => setBusy(false)); }}><Link size={14} />{t("复制比较链接", "Copy compare link")}</button></div></div>}
      {peer && !profile && <div className="empty-state"><h2>{t("先建立你的艺术人格画像", "Create your own artistic profile")}</h2><p style={{ marginBottom: 16, color: "var(--muted)", fontSize: 13 }}>{t("选择对方的一个榜单，用其中的作品创建你自己的排序。", "Pick one of their rankings to create your own.")}</p><button className="button primary" onClick={() => setPeerRankPickOpen(true)}><Play size={16} />{t("选择对方榜单", "Pick their ranking")}</button></div>}
  ...
  return <div className="app-shell"><header className="topbar"><button className="wordmark" onClick={() => navigateTo("home")}>ART<span>/</span>RANK</button><nav aria-label={t("主导航", "Main navigation")}><button className={view === "home" || view === "source" || view === "setup" || view === "sorting" ? "active" : ""} onClick={() => navigateTo("home")}>{t("清单", "Catalog")}</button><button className={view === "compare" ? "active" : ""} onClick={() => navigateTo("compare")}>{t("相遇", "Encounter")}</button>{profile && <button className={view === "profile" ? "active" : ""} onClick={() => navigateTo("profile")}>{t("我的文化索引", "My Index")}</button>}</nav><div className="header-tools"><IconButton title={locale === "zh" ? "English" : "中文"} onClick={() => { const next = locale === "zh" ? "en" : "zh"; setLocale(next); const url = new URL(location.href); url.searchParams.set("lang", next); history.replaceState(null, "", url); }}><Languages size={18} /></IconButton><a className="icon-button" href="https://github.com/tripodxu/film-sort" target="_blank" rel="noopener noreferrer" title="GitHub"><Github size={18} /><span className="tooltip" role="tooltip">GitHub</span></a><IconButton title={t("登录 / 同步", "Sign in / Sync")} onClick={() => setAccountOpen(true)}><UserRound size={18} />{accountToken && <span style={{ position: "absolute", top: 4, right: 4, width: 7, height: 7, borderRadius: "50%", background: syncStatus === "saving" ? "var(--yellow)" : syncStatus === "saved" ? "var(--green)" : syncStatus === "error" ? "var(--red)" : "var(--muted)", border: "1.5px solid var(--bg)", zIndex: 1 }} />}</IconButton></div></header>
    ...
    {compareRankDetail && <div className="modal-backdrop" onClick={() => setCompareRankDetail(null)}><section className="rank-detail-dialog" role="dialog" aria-modal="true" onClick={(event) => event.stopPropagation()} onKeyDown={(event) => { if (event.key === "Escape") setCompareRankDetail(null); }}><div className="section-heading"><div><span className="eyebrow">{compareRankDetail.side === "own" ? t("我的索引", "MY INDEX") : t("对方索引", "THEIR INDEX")} / {label(compareActiveKind)}</span><h2>{compareRankDetail.collectionTitle}</h2></div><IconButton title={t("关闭", "Close")} onClick={() => setCompareRankDetail(null)}><X size={18} /></IconButton></div>{compareRankDetail.ranking ? <ol className="ranking-list">{compareRankDetail.ranking.items.map((work) => { const isHit = compareRankDetail.highlightId === work.id; return <li key={work.id} style={isHit ? { background: "rgba(216,248,106,.08)", borderLeft: "3px solid var(--accent)", paddingLeft: 14 } : {}}><span className="row-number" style={isHit ? { color: "var(--accent)" } : {}}>{String(work.rank).padStart(2, "0")}</span><Poster work={work} kind={compareRankDetail.ranking!.kind} /><div><strong>{work.title}</strong><small>{work.creator} {work.year}</small></div></li>; })}</ol> : <p className="empty-state">{t("无法找到该榜单数据", "Ranking data not found")}</p>}</section></div>}
