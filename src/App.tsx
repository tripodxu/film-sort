...
<div className="profile-layout"><section><div className="section-heading">{isRenamingProfile ? <div style={{ display: "flex", gap: 6, alignItems: "center" }}><input type="text" value={editingRankTitle} onChange={(e) => setEditingRankTitle(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") renameRank(profileRankIdx); if (e.key === "Escape") setEditingRankIdx(null); }} style={{ fontSize: 18, fontWeight: 600, padding: "4px 8px", minHeight: "auto", flex: 1 }} autoFocus /><button className="text-button" onClick={() => renameRank(profileRankIdx)} style={{ color: "var(--accent)", fontSize: 14 }}>✓</button></div> : <h2>{activeRanking.collectionTitle}</h2>}<div style={{ display: "flex", gap: 8, alignItems: "center" }}><span>{new Date(activeRanking.createdAt).toLocaleDateString(locale === "zh" ? "zh-CN" : "en-US")}</span><button className="text-button" onClick={() => { setEditingRankIdx(profileRankIdx); setEditingRankTitle(activeRanking.collectionTitle); }} style={{ fontSize: 12, color: "var(--muted)" }}>{t("改名", "Rename")}</button><button className="text-button" onClick={() => { const works = activeRanking.items.map(item => ({ id: item.id, title: item.title, subtitle: item.subtitle, creator: item.creator, year: item.year, posterUrls: item.posterUrls })); openCollection({ id: `rerank-${activeRanking.profileId}`, kind: activeRanking.kind, source: "custom", title: activeRanking.collectionTitle, description: "", topN: activeRanking.items.length, works }); }} style={{ fontSize: 12, color: "var(--accent)" }}>{t("重新排序", "Re-rank")}</button></div></div><ol className="ranking-list">{activeRanking.items.map((work) => <li key={work.id}><span className="row-number">{String(work.rank).padStart(2, "0")}</span><Poster work={work} kind={activeRanking.kind} /><div><strong>{work.title}</strong><small>{work.creator} {work.year}</small></div>{work.rank <= 3 && <span style={{ fontSize: work.rank === 1 ? 20 : 16 }}>{work.rank === 1 ? "🥇" : work.rank === 2 ? "🥈" : "🥉"}</span>}</li>)}</ol></section><aside className="export-tools"><label htmlFor="profile-name">{t("画像名称", "Profile name")}</label><input id="profile-name" value={profileName} maxLength={80} onChange={(event) => { setProfileName(event.target.value); setShareUrl(""); setQrUrl(""); }} onBlur={() => { const next = namedProfile(); if (next) persist(next); }} /><label htmlFor="export-format">{t("导出格式", "Export format")}</label><select id="export-format" value={format} onChange={(event) => setFormat(event.target.value as typeof format)}>{["json", "png", "txt", "csv", "md"].map((item) => <option key={item} value={item}>{item.toUpperCase()}</option>)}</select><button className="button primary" onClick={exportProfile}><Download size={16} />{t("导出全部维度", "Export all media")}</button><button className="button secondary" onClick={share}><Share2 size={16} />{t("复制比较链接", "Copy comparison link")}</button>{shareUrl && <div className="share-output"><input aria-label={t("比较链接", "Comparison link")} readOnly value={shareUrl} onFocus={(event) => event.target.select()} />{qrUrl && <img src={qrUrl} alt={t("比较二维码", "Comparison QR code")} />}</div>}<button className="button quiet" onClick={() => setView("home")}><Plus size={16} />{t("添加另一个维度", "Add another medium")}</button><button className="button quiet" onClick={() => setView("compare")}><Users size={16} />{peer ? t("继续与好友比较", "Continue comparison") : t("与他人比较", "Compare with someone")}</button>{ranking?.completed && collection?.kind === activeRanking.kind && <button className="button quiet" onClick={() => { setRanking(undoLastAction(ranking)); setView("sorting"); }}><Undo2 size={16} />{t("返回最后一次取舍", "Revisit last choice")}</button>}</aside></div>
    ...
    content = <>{heading(t("相遇", "ENCOUNTER"), t("看看你们的选择在哪里重合", "See where your choices meet"), peer ? t("两份索引的交集与分歧", "Overlap and divergence of two indexes") : t("分享你的索引，或导入对方的来比较。", "Share your index or import theirs to compare."))}{peer && <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 12, gap: 8 }}><button className="text-button" onClick={() => { setPeer(null); setView("home"); setAiInsight(""); setCompareActiveKind("film"); setCompareMode("auto"); setManualOwnSelections(new Set()); setManualPeerSelections(new Set()); }} style={{ fontSize: 12, color: "var(--muted)" }}><X size={14} style={{ verticalAlign: "middle", marginRight: 4 }} />{t("退出比较", "Exit comparison")}</button></div>}{peer && profile && <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16, flexWrap: "wrap" }}><span style={{ fontSize: 11, color: "var(--muted)" }}>{t("比较模式", "Mode")}:</span><div className="segmented" style={{ marginBottom: 0 }}><button role="tab" aria-selected={compareMode === "auto"} className={compareMode === "auto" ? "active" : ""} onClick={() => setCompareMode("auto")}>{t("自动合并", "Auto merge")}</button><button role="tab" aria-selected={compareMode === "manual"} className={compareMode === "manual" ? "active" : ""} onClick={() => setCompareMode("manual")}>{t("指定榜单", "Pick lists")}</button></div>{compareMode === "manual" && <div style={{ display: "flex", gap: 24, marginBottom: 8, flexWrap: "wrap" }}><div><span style={{ fontSize: 11, color: "var(--accent)", fontWeight: 600 }}>{t("我的榜单", "My lists")}</span><div style={{ display: "flex", flexDirection: "column", gap: 4, marginTop: 4 }}>{ownRankings.map((r, i) => <label key={i} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12, cursor: "pointer", color: manualOwnSelections.has(i) ? "var(--text)" : "var(--muted)" }}><input type="checkbox" checked={manualOwnSelections.has(i)} onChange={() => { const next = new Set(manualOwnSelections); next.has(i) ? next.delete(i) : next.add(i); setManualOwnSelections(next); }} />{r.collectionTitle}</label>)}</div></div><div><span style={{ fontSize: 11, color: "var(--accent)", fontWeight: 600 }}>{t("对方榜单", "Their lists")}</span><div style={{ display: "flex", flexDirection: "column", gap: 4, marginTop: 4 }}>{peerRankings.map((r, i) => <label key={i} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12, cursor: "pointer", color: manualPeerSelections.has(i) ? "var(--text)" : "var(--muted)" }}><input type="checkbox" checked={manualPeerSelections.has(i)} onChange={() => { const next = new Set(manualPeerSelections); next.has(i) ? next.delete(i) : next.add(i); setManualPeerSelections(next); }} />{r.collectionTitle}</label>)}</div></div></div>}</div>}<div className="comparison-inputs"><section><span className="eyebrow">01 / {t("我的索引", "MY INDEX")}</span><h2>{profile?.profileName ?? t("尚未创建", "Not created")}</h2>{profile ? <div style={{ marginTop: 8, display: "flex", flexDirection: "column", gap: 4 }}>{profile.rankings.map((entry, idx) => <div key={idx} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13 }}><span style={{ color: "var(--accent)", fontSize: 11, fontWeight: 600, width: 28, flexShrink: 0 }}>{label(entry.kind)}</span><Poster work={entry.items[0]} kind={entry.kind} /><span style={{ flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{entry.collectionTitle}</span><span style={{ color: "var(--muted)", fontSize: 11, flexShrink: 0 }}>TOP {entry.items.length}</span></div>)}</div> : fileInput("own", t("导入我的索引", "Import my index"))}</section><section><span className="eyebrow">02 / {t("对方索引", "THEIR INDEX")}</span><h2>{peer?.profileName ?? t("等待导入", "Awaiting import")}</h2>{peer ? <div style={{ marginTop: 8, display: "flex", flexDirection: "column", gap: 4 }}>{peer.rankings.map((entry, idx) => <div key={idx} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13 }}><span style={{ color: "var(--accent)", fontSize: 11, fontWeight: 600, width: 28, flexShrink: 0 }}>{label(entry.kind)}</span><Poster work={entry.items[0]} kind={entry.kind} /><span style={{ flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{entry.collectionTitle}</span><span style={{ color: "var(--muted)", fontSize: 11, flexShrink: 0 }}>TOP {entry.items.length}</span></div>)}</div> : <div style={{ marginTop: 8, display: "flex", flexDirection: "column", gap: 8 }}><div style={{ display: "flex", gap: 6 }}><input type="url" placeholder={t("粘贴比较链接或 JSON 地址…", "Paste compare link or JSON URL…")} value={peerUrl} onChange={(e) => setPeerUrl(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") void importPeerFromUrl(); }} style={{ flex: 1, minHeight: 38, fontSize: 13 }} /><button className="button secondary" disabled={!peerUrl.trim() || peerUrlBusy} onClick={() => void importPeerFromUrl()} style={{ minHeight: 38, padding: "0 12px" }}>{peerUrlBusy ? "…" : t("导入", "Import")}</button></div>{fileInput("peer", t("或上传 JSON 文件", "or upload JSON"))}</div>}</section></div>
