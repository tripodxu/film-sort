...
export default function App() {
  ...
  if (view === "home") content = <>
    ...
    <section className="profile-overview"><div className="section-heading"><h2>{t("品味年轮", "Taste Rings")}</h2><div style={{ display: "flex", gap: 12, alignItems: "center" }}>{profile && <button className="text-button" onClick={() => setView("profile")}>{t("查看画像", "View profile")}<ArrowRight size={15} /></button>}{profile && <button className="button quiet" onClick={clearAllData} style={{ fontSize: 11, padding: "4px 10px", borderRadius: 8, border: "1px solid var(--line)", color: "var(--muted)" }}>🗑 {t("清除数据", "Clear")}</button>}</div></div>{profile ? <div className="coordinate-grid">{profile.rankings.map((entry, idx) => { const isActive = editingRankIdx === idx; return <div key={`${entry.kind}-${entry.collectionTitle}-${idx}`} className="coordinate" style={{ position: "relative" }}><div style={{ display: "flex", gap: 6, position: "absolute", top: 6, right: 6, zIndex: 2 }}><button onClick={(e) => { e.stopPropagation(); setEditingRankIdx(idx); setEditingRankTitle(entry.collectionTitle); }} title={t("重命名", "Rename")} style={{ width: 12, height: 12, borderRadius: "50%", background: "#f5c542", border: "1px solid #d4a830", cursor: "pointer", padding: 0 }} /><button onClick={(e) => { e.stopPropagation(); deleteRank(idx); }} title={t("删除", "Delete")} style={{ width: 12, height: 12, borderRadius: "50%", background: "#ff5f57", border: "1px solid #e04842", cursor: "pointer", padding: 0 }} /></div><button onClick={() => { setActiveKind(`${entry.kind}-${idx}`); setView("profile"); }} style={{ display: "flex", gap: 14, alignItems: "center", flex: 1, background: "none", border: "none", color: "inherit", cursor: "pointer", padding: 0, textAlign: "left" }}><Poster work={entry.items[0]} kind={entry.kind} /><div><span>{label(entry.kind)}</span>{isActive ? <div style={{ display: "flex", gap: 4, marginTop: 4 }}><input type="text" value={editingRankTitle} onChange={(e) => setEditingRankTitle(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") renameRank(idx); if (e.key === "Escape") setEditingRankIdx(null); }} style={{ fontSize: 13, padding: "2px 6px", minHeight: "auto", width: "100%" }} autoFocus /><button className="text-button" onClick={(e) => { e.stopPropagation(); renameRank(idx); }} style={{ color: "var(--accent)", fontSize: 11 }}>{t("✓", "✓")}</button></div> : <h3>{entry.collectionTitle}</h3>}<small>{entry.items.length} {t("件作品", "works")}</small></div></button></div>; })}</div> : <div className="empty-profile"><div className="sample-covers" aria-hidden="true">{getCollectionsByKind("film")[1]?.works.filter((work) => work.posterUrls?.length).slice(0, 3).map((work) => <Poster key={work.id} work={work} kind="film" />)}</div><div><h3>{t("还没有完成的画像", "No completed profile yet")}</h3><span>{t("电影 · 书籍 · 音乐 · 其他", "Films · Books · Music · Other")}</span></div>{fileInput("own", t("导入我的画像", "Import my profile"))}</div>}</section>
  </>;
