...
export default function App() {
  ...
  if (view === "home") content = <>
    ...
    <section className="medium-section"><div className="section-heading"><h2>{t("选择一个维度", "Choose a medium")}</h2><span>01 / 04</span></div><div className="medium-grid">{kinds.map((item) => { const Icon = icons[item]; const saved = profile?.rankings.filter((entry) => entry.kind === item) ?? []; const latest = saved[0]; const builtinWorks = getCollectionsByKind(item).flatMap(c => c.works).filter(w => w.posterUrls?.length); const randomWork = builtinWorks.length > 0 ? builtinWorks[Math.floor(Math.random() * builtinWorks.length)] : null; return <button className={`medium-item medium-${item}`} key={item} onClick={() => chooseKind(item)}><span className="medium-number">{mediaLabels[item].symbol}</span>{latest ? <Poster work={latest.items[0]} kind={item} /> : randomWork ? <Poster work={randomWork} kind={item} /> : <Icon size={20} />}<strong>{label(item)}</strong><small>{saved.length > 0 ? `${saved.length}${t("个榜单", "lists")} / ${saved.reduce((s, r) => s + r.items.length, 0)}${t("件", "")}` : t(mediaLabels[item].description, "New ranking")}</small><ArrowRight size={16} /></button>; })}</div></section>
