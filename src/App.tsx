...
export default function App() {
  ...
  if (view === "home") content = <>
    ...
    <div className="profile-dimensions">{profile.rankings.map((entry) => <button className={`profile-dimension medium-${entry.kind} ${activeRanking.kind === entry.kind ? "active" : ""}`} key={entry.kind} onClick={() => setActiveKind(entry.kind)}><span>{label(entry.kind)}</span><strong>{entry.items[0].title}</strong><small>TOP {entry.items.length}</small></button>)}</div>
