...
import { gdPlay, gdLyric, buildLyricLines, type GdErrorInfo, type LyricResult } from "../lib/gdMusic";
...
export function ArtworkDetail({ detail, label, t, onClose }: { detail: ArtworkDetailInfo; label: (kind: MediaKind) => string; t: (zh: string, en: string) => string; onClose: () => void }) {
  ...
  return <div className="modal-backdrop" style={{ zIndex: 60 }} onClick={onClose}><section className="detail-dialog" role="dialog" aria-modal="true" aria-labelledby="detail-heading" onClick={(event) => event.stopPropagation()}><div className="section-heading"><div><span className="eyebrow">{label(detail.kind)} / {t("作品详情", "WORK DETAIL")}</span><h2 id="detail-heading">{detail.work.title}</h2></div><IconButton title={t("关闭", "Close")} onClick={onClose}><X size={18} /></IconButton></div><div className="detail-body"><Poster work={detail.work} kind={detail.kind} large /><div className="detail-copy">{isMusic && <div className="music-preview"><button className="button secondary" disabled={playBusy} onClick={() => void playMusic()}><Play size={15} />{playBusy ? t("准备试听…", "Preparing…") : t("试听", "Listen")}</button><button className="button secondary" disabled={lyricBusy} onClick={() => void loadLyric()}><Music2 size={15} />{lyricBusy ? t("加载歌词…", "Loading…") : lyrics === null ? t("歌词", "Lyrics") : t("收起歌词", "Hide lyrics")}</button>{playUrl && <audio controls autoPlay src={playUrl} />}{playError && <small className="music-panel-msg">{playError}</small>}{isCover && playUrl && <small className="music-panel-msg" style={{ color: "var(--warn)" }}>{t("未找到原版，仅提供翻唱试听", "Original not found — playing a cover version")}</small>}{lyricError && <small className="music-panel-msg">{lyricError}</small>}{lyrics && <div className="music-lyric">{buildLyricLines(lyrics.lyric, lyrics.tlyric).map((line, index) => <p key={index} className="music-lyric-line">{line.text ? <span>{line.text}</span> : null}{line.translation ? <small className="music-lyric-translation">{line.translation}</small> : null}</p>)}</div>}</div>}{detail.loading ? <p className="empty-state">{isMusic ? t("正在读取专辑信息…", "Loading album details…") : t("正在读取作品信息…", "Loading work details…")}</p> : detail.data ? <><div className="detail-meta"><span>{detail.work.creator ?? ""}</span><span>{detail.work.year ?? ""}</span><span>{String(detail.data.rating ?? "")}</span></div>{typeof detail.data.content_intro === "string" && detail.data.content_intro ? <div className="detail-synopsis"><h3>{t("简介", "Synopsis")}</h3><p>{detail.data.content_intro}</p>{typeof detail.data.content_source === "string" && <span className="detail-source">— {detail.data.content_source}</span>}</div> : <p style={{ fontSize: 13, color: "var(--muted)", marginTop: 12 }}>{t("暂无简介，数据来源暂未收录该作品。", "No synopsis available yet.")}</p>}<dl>{Object.entries(detail.data).filter(([key, value]) => value && !["title", "pic", "rating", "imgs", "content_intro", "content_source"].includes(key)).slice(0, 8).map(([key, value]) => <div key={key}><dt>{key}</dt><dd>{Array.isArray(value) ? value.join("、") : String(value)}</dd></div>)}</dl></> : <p className="empty-state">{t("暂时没有更多资料，仍可保留这件作品。", "No additional details were found.")}</p>}</div></div></section></div>;
}
