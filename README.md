...
两种链接在「我的文化索引」和「相遇」页面均可生成，均包含批注数据。
...
- **批注系统**：文化索引页面支持三个层级的批注——画像整体（胶囊按钮「添加画像批注」）、榜单标题（胶囊按钮「添加批注」）、单个作品（行内便签图标）。有批注时按钮高亮 accent 色并显示文字。登录状态下批注随画像自动同步云端。分享链接、查看分享页、广场帖子详情、比较详情弹窗均支持批注展示（长批注截断后点击弹窗查看完整内容）。导出（TXT/MD/CSV/JSON）均自动包含批注内容。页面刷新或重新登录时自动从云端恢复批注。
