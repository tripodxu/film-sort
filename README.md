...
- **批注系统**：文化索引页面支持三个层级的批注——画像整体（胶囊按钮「添加画像批注」）、榜单标题（胶囊按钮「添加批注」）、单个作品（行内便签图标）。有批注时按钮高亮 accent 色并显示文字。登录状态下批注随画像自动同步云端。分享链接和导出（TXT/MD/CSV/JSON）均自动包含批注内容。
- **榜单详情弹窗优化**：共同作品和比较界面点击排名打开的详情弹窗，前三名显示金银铜牌奖牌。
