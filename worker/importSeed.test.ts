 ...
 */
function statement(sql: string, options: { user?: { id: number; email: string } | null }, args: unknown[]) {
  return {
    first: async () => {
    ...
    all: async () => ({ results: [] }),
    run: async () => { captures.push({ sql, args }); return { success: true, meta: {} }; },
...
describe("seedImportedPosterUrls（共用实现）", () => {
  it("音乐：网易云封面按 type|title|title| 落库", async () => {
    await seedImportedPosterUrls(env(fakeDb()), [
      { id: "netease-1", title: "童话", creator: "光良", type: "music", poster_url: "https://p2.music.126.net/abc==/1.jpg" },
    ]);
    expect(seededRows()).toEqual([{ key: "music|童话|童话|", urls: ["https://p2.music.126.net/abc==/1.jpg"] }]);
  });

  it("电影 / 书籍：键里带上年份，与客户端发送的键一致", async () => {
    const works: ImportedWork[] = [
      { id: "d1", title: "霸王别姬", year: 1993, type: "movie", poster_url: "https://img1.doubanio.com/view/subject/m/public/s1.jpg" },
      { id: "d2", title: "活着", year: 1993, type: "book", poster_url: "https://img3.doubanio.com/view/subject/m/public/s2.jpg" },
    ];
  ...
  it("丢弃不可用地址：非白名单主机、空值、豆瓣静态占位图", async () => {
    await seedImportedPosterUrls(env(fakeDb()), [
      ...
      { id: "d", title: "D", type: "movie", poster_url: "https://img1.doubanio.com/f/shire/abc.jpg" },
      { id: "e", title: "E", type: "movie", poster_url: "https://img1.doubanio.com/view/subject/m/public/ok.jpg" },
    ]);
  ...
  it("缺 type 时用清单媒介兜底；仍然缺 type 就跳过（避免写下永远取不到的键）", async () => {
    await seedImportedPosterUrls(env(fakeDb()), [
      { id: "a", title: "无类型", poster_url: "https://img1.doubanio.com/view/subject/m/public/a.jpg" },
    ], "book");
    expect(seededRows().map((row) => row.key)).toEqual(["book|无类型|无类型|"]);

    captures.length = 0;
    await seedImportedPosterUrls(env(fakeDb()), [
      { id: "b", title: "无类型也没兜底", poster_url: "https://img1.doubanio.com/view/subject/m/public/b.jpg" },
    ]);
    expect(seededRows()).toHaveLength(0);
  });

  it("无 DB / 空清单时安静返回", async () => {
    await expect(seedImportedPosterUrls(env(undefined as unknown as D1Database), [])).resolves.toBeUndefined();
...
describe("三条导入路径都接上了种子化", () => {
  it("网易云歌单（音乐）", async () => {
    vi.stubGlobal("fetch", async (input: RequestInfo | URL) => {
      ...
      if (url.hostname === "music.163.com" && url.pathname.includes("song/detail")) {
        return json({ songs: [{ id: 111, name: "童话", ar: [{ name: "光良" }], al: { picUrl: "https://p2.music.126.net/x==/3.jpg" } }] });
      }
      return json({}, 502);
    });

    const response = await importRoute(new Request("https://example.com/api/import/netease?url=123456", { headers: auth }), env(fakeDb({ user: { id: 1, email: "a@b.c" } })));
    expect(response.status).toBe(200);
    expect(seededRows()).toEqual([{ key: "music|童话|童话|", urls: ["https://p2.music.126.net/x==/3.jpg"] }]);
  });
  ...
  it("豆瓣书影音清单（电影/书籍，rexxar JSON 接口）", async () => {
      ...
      if (url.hostname === "m.douban.com") {
        return json({
          ...
          subject_collection_items: [
            { id: "1291546", title: "霸王别姬", year: "1993", type: "movie", info: "陈凯歌 / 1993", cover: { large: "https://img1.doubanio.com/view/subject/l/public/s1.jpg" } },
            { id: "1084336", title: "活着", year: "1993", type: "book", info: "余华 / 1993", cover: { large: "https://img3.doubanio.com/view/subject/l/public/s2.jpg" } },
          ],
        });
      }
      return json({}, 502);
    ...
    const response = await importRoute(
      new Request("https://example.com/api/import/douban-list?url=https%3A%2F%2Fm.douban.com%2Fsubject_collection%2Fmovie_top250%2F", { headers: auth }),
      env(fakeDb({ user: { id: 1, email: "a@b.c" } })),
    );
    expect(response.status).toBe(200);
    expect(seededRows().map((row) => row.key)).toEqual(["movie|霸王别姬|霸王别姬|1993", "book|活着|活着|1993"]);
  });
  ...
  it("响应体形状不变：种子化不改变客户端看到的 works", async () => {
    vi.stubGlobal("fetch", async (input: RequestInfo | URL) => {
      const url = new URL(typeof input === "string" ? input : input instanceof URL ? input.href : input.url);
      if (url.hostname === "m.douban.com") {
        return json({ total: 1, subject_collection_items: [{ id: "1", title: "霸王别姬", year: "1993", type: "movie", cover: { large: "https://img1.doubanio.com/view/subject/l/public/s1.jpg" } }] });
      }
      return json({}, 502);
    });
    ...
    expect(body.works[0]).toMatchObject({ title: "霸王别姬", poster_url: "https://img1.doubanio.com/view/subject/l/public/s1.jpg", type: "movie" });
