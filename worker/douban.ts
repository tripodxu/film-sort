...
function jarHeader(code: string): string | null {
  ...
  if (Date.now() - touched > JAR_TTL_MS) {
    qrJar.delete(code);
    jarTouched.delete(code);
    return null;
  }
  return [...jar.entries()].map(([k, v]) => `${k}=${v}`).join("; ");
}

function jarDrop(code: string) {
  qrJar.delete(code);
  jarTouched.delete(code);
}
...
export interface DoubanQrIssue {
  code: string;
  qrImage: string;
  ttl: number;
}
...
export async function doubanQrPoll(
  code: string,
  env: Env,
  userId: number,
  ...
  if (loginStatus === "login") {
    ...
    if (!dbcl2) {
      jarDrop(code);
      return { state: "expired", error: "登录已确认但未能获取凭证（dbcl2），请重新扫码" };
    }
    const ck = jar?.get("ck");
  ...
  return {
    state: "risk",
    error: `豆瓣返回了未知状态（${loginStatus || data.status || "empty"}），请重新扫码`,
  };
}
...
>();
export async function doubanAccountInfo(
  env: Env,
  userId: number,
