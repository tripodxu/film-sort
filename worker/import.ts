...
async function doulistPage(url: string, cookie?: string | null): Promise<DoulistItem[]> {
  ...
  const rewritten = new HTMLRewriter()
    ...
    .on("div.doulist-item .abstract", {
      text(chunk) {
        if (current) current.pub = (current.pub ?? "") + chunk.text;
      },
    })
    .on("div.doulist-item .abstract br", {
      element() {
        if (current) current.pub = (current.pub ?? "") + "\n";
      },
    })
    .on("div.doulist-item .rating_nums", {
      text(chunk) {
        if (current) current.rating = (current.rating ?? "") + chunk.text;
      },
    })
    // ===== 旧版式（兜底） =====
    .on("table.olt tr.item", {
      element() {
        finish();
        current = { title: "", url: "" };
      },
    ...
    .on("table.olt tr.item td:nth-child(2) img", {
      element(el) {
        if (current && !current.poster) current.poster = el.getAttribute("src") ?? undefined;
      },
    })
    .on("table.olt tr.item td:nth-child(3)", {
      text(chunk) {
        if (current) current.pub = (current.pub ?? "") + chunk.text;
      },
    })
    .on("table.olt tr.item .rating_nums", {
      text(chunk) {
        if (current) current.rating = (current.rating ?? "") + chunk.text;
      },
    })
...
function doulistCreator(pub: string): string | undefined {
  ...
  for (const line of lines) {
    const match = line.match(
      /^(?:作者|原著|原作|著者|译者|导演|编剧|歌手|演员|主演|演唱者|艺术家|艺人|乐队|组合|作曲家)\s*[:：]\s*(.+)$/,
    );
...
export async function fetchDoulist(
  doulistUrl: string,
  cookie?: string | null,
  offset = 0,
  limit = 300,
  ...
  for (
    let pageStart = Math.floor(offset / 25) * 25;
    works.length < limit && pageStart < offset + limit + 25;
    pageStart += 25
  ) {
    const items = await doulistPage(
      `https://www.douban.com/doulist/${id}/?start=${pageStart}&sort=seq`,
      cookie,
    ).catch(() => []);
    ...
    for (let idx = 0; idx < items.length; idx++) {
      ...
      const creator =
        doulistCreator(item.pub ?? "") ??
        (() => {
          const pubParts = (item.pub ?? "")
            .split("/")
            .map((s) => s.trim())
            .filter(Boolean);
...
export async function fetchNeteasePlaylist(
    ...
    async () =>
      (
        await weapiPost(
          ...
          { id: playlistId, n: 1000, s: 0 },
          cookie ?? null,
        )
      ).json.playlist as DetailShape | undefined,
    async () =>
      (
        await weapiPost(
          "/weapi/v6/playlist/detail/",
  ...
  for (let start = 0; start < windowIds.length; start += 300) {
    ...
    const got = await firstNonEmpty<NeteaseSong>([
      ...
      async () =>
        ((await weapiPost("/weapi/v3/song/detail/", { c: cJson, ids: idsJson }, cookie ?? null))
          .json.songs as NeteaseSong[] | undefined) ?? [],
      async () =>
        ((await weapiPost("/weapi/v3/song/detail/", { c: cJson, ids: idsJson }, cookie ?? null))
          .json.songs as NeteaseSong[] | undefined) ?? [],
    ]);
  ...
  const receivedIds = new Set(
    songs
      .map((song) => song.id)
      .filter((id): id is number => typeof id === "number" && requestedIds.has(id)),
  );
  const firstMissingIndex = windowIds.findIndex((id) => !receivedIds.has(id));
  const nextOffset =
    firstMissingIndex >= 0 ? offset + firstMissingIndex : offset + windowIds.length;
  const hasMore =
    firstMissingIndex >= 0 || (total != null ? nextOffset < total : nextOffset < allIds.length);
  if (fromSongs.length)
    return { works: fromSongs, total: total ?? (allIds.length || null), hasMore, nextOffset };
  // 全部失败时回退 detail 自带的部分 tracks（至少能拿到前几首）
  const fallback = fallbackTracks
...
export async function importRoute(request: Request, env: Env): Promise<Response> {
  ...
  if (url.pathname === "/api/import/doulist" && request.method === "GET") {
    ...
    if (!/^https:\/\/(www\.)?douban\.com\/doulist\/\d+\/?/.test(target)) {
      return json(
        {
          error: "invalid_doulist_url",
          msg: "请粘贴豆瓣豆列链接，如 https://www.douban.com/doulist/12345/",
        },
        400,
      );
    }
    try {
      const page = await fetchDoulist(
        target,
        await loadProviderCookie(env, user.id, "douban"),
        offset,
        limit,
      );
      if (!page.works.length)
        return json({ error: "doulist_empty", msg: "该豆列为空或抓取被拦截，请稍后重试" }, 502);
      await seedImportedPosterUrls(env, page.works);
      return json({
        works: page.works,
        total: page.works.length,
      ...
      return json(
        { error: "doulist_unavailable", msg: "豆列抓取失败，豆瓣可能限流，请稍后重试" },
        502,
      );
    }
  }
  ...
  if (url.pathname === "/api/import/douban-list" && request.method === "GET") {
    ...
    if (classified.kind === "unknown") {
      return json(
        {
          error: "invalid_url",
          msg: "请粘贴豆瓣豆列（doulist/123）、我的书影音（mine?status=wish/collect）或清单（subject_collection/XXX）链接",
        },
        400,
      );
    }
    try {
      ...
      const page: PagedImport & { error?: string } =
        classified.kind === "doulist"
          ? await fetchDoulist(target, cookie, offset, limit)
          : await fetchDoubanList(classified, cookie, offset, limit);
      if (page.error === "not_connected")
        return json(
          {
            error: "not_connected",
            msg: "请先在上方连接豆瓣（扫码或粘贴 Cookie），再导入想看/已看清单",
          },
          400,
        );
      if (!page.works.length) {
      ...
      return json(
        { error: "douban_list_unavailable", msg: "豆瓣清单抓取失败，可能限流，请稍后重试" },
        502,
      );
    }
  }
  ...
  if (url.pathname === "/api/import/netease" && request.method === "GET") {
      ...
      return json(
        {
          ...
          msg: "请粘贴网易云歌单链接（含 playlist?id=）或纯歌单 ID",
        },
        400,
      );
    }
    try {
      ...
      if (!page.works.length)
        return json({ error: "playlist_empty", msg: "该歌单为空，请确认链接后重试" }, 502);
      await seedImportedPosterUrls(env, page.works);
      return json({
        works: page.works,
        total: page.works.length,
      ...
      return json(
        { error: "netease_unavailable", msg: "歌单抓取失败，网易接口可能限流，请稍后重试" },
        502,
      );
    }
  }
  ...
  if (url.pathname === "/api/import/netease/mine" && request.method === "GET") {
    const cookie = await loadProviderCookie(env, user.id, "netease");
    if (!cookie)
      return json({ error: "not_connected", msg: "尚未连接网易云账号，请先扫码登录" }, 400);
    const uid = await neteaseUserId(cookie);
    if (!uid)
      return json({ error: "connection_expired", msg: "网易云连接已过期，请重新扫码登录" }, 401);
