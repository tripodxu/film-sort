...
3. **创作者组合搜索** — 如"三体 刘慈欣"；
4. **百度百科** — 兜底；**中文歌名（音乐详情）优先于维基**，因百科词条名通常就是歌名本身，对华语流行单曲覆盖远好于维基。传输为三级探测：开放 API（共享 demo appid，2026-09 起持续 `errno:6`）→ 词条页 meta description（百度对数据中心 IP 常下安全验证页，只读响应头部 96KB）→ **anysearch 搜索**（`api.anysearch.com/mcp`，JSON-RPC `tools/call search`，取词条页结果摘要；第三方服务，匿名额度，失败静默回落）。命中结果进 isolate 级简介缓存（FIFO 200 条，TTL 24h，仅缓存成功值）。
