...
> 数值已由 P0.7 两屏**定案**（2026-09-22，§10.7 执行记录）：玻璃两级（贵级 `blur(16px)+saturate(1.2)` / 普级 `blur(8px)`）、噪点 4.5%、`--ls-wide:.22em`、半调网点 4px 周期、卡片 `--r-sm`、海报 `--r-xs`、`--ease-out:cubic-bezier(.22,.61,.36,1)`。**P1a.5 对比度定案**：`--text-3` 混合 **74%→78%**（classic/surface3 最差点 4.11→4.62:1，最小达标值；报告 `docs/ui-contrast.json`）。
...
| P1a | Token 声明（5 档圆角/间距/字阶/两档字号/L2）+ **散落 hex 归零（17 处）** + kind 徽章 token 引用核验 + 媒介色越界使用回收（前置：§2.5 去重已完成）（纯增量、**零 diff 可证**） | 极低 | 四门禁 + **零 diff 快照（seed 模式 `--tol 0` 严格零；非 seed `--tol 8`，见 §8.0）** | 0.5d（✅ 完成 2026-09-22，270/270 零 diff，§10.7） |
| P1a.5 | **浅色四主题对比度扫描与 `--text-3` 定案（四轮 #三.6，纯脚本）**：retro/minimal/simple/classic 的 `color-mix` surface + `--text-3`（74% 混合，承担 44×11px + 44×12px meta）从未做过 4.5:1 验证——结果决定是否上调混合比；**若上调属 L1 token 变更，必须在 P1b 前定案**，否则 P1b/P2/P3 全部基线重拍。本计划唯一"先后顺序显著影响成本"的依赖 | 低 | 对比度报告（组合×字号矩阵）+ L1 定案记录 | 0.5d（✅ 完成 2026-09-22，78% 定案，§10.7） |
| P1b | 裸数值归一 + 字体栈 + 噪点（§2 余下）——**顺序：圆角收敛先行**（12 值→5 档、弹层统一单档）**再间距**（三轮 #一.1）；交付物明列 **玻璃两级制（贵级 2 类 + 普级 2 类，其余 17 类退哑光）**、阴影 36→2、动效曲线统一（含 jelly 残留清理） | 低（有容差位移） | 四门禁 + **B0→B1** 快照按 §2.1 容差判 + 动效节奏录屏 + 移动端滚动帧率手测 | 1.5d（✅ 完成 2026-09-22，B1 落地 + 判读 PASS，§10.7） |
| P2 | 壳层：topbar/footer/toast/弹层体系（§3）+ **help 入口迁 topbar**（§0 第六类，四轮 #三.5，一次解决触控/遮挡/发现性三件事） | 低 | 四门禁 + 弹层逐个开合手测 + help 发现性手测 + toast 位置定案 | 1d（✅ 源码完成 2026-09-22；弹层开合实测并入 P4，§10.7） |
...
| P5c | 布局模式系统（§5，原 P4.5 **后移**——主题人格是纯 CSS 覆写先做，布局是唯一新增 JS+DOM 后做，2026-09 审查 #五）：`layout.ts` + 切换 UI + journey/bento CSS + 装饰包裹 div | 中偏高（唯一新增 JS + 唯一新增 DOM） | 四门禁 + 18 组合抽样（脚本化）+ 键盘/reduced-motion 专项 + **模式对比基线**核对 | 2.5d（✅ 完成 2026-09-22，§10.7） |
| 发布前 | **真机触控人工验收**（iOS Safari + Android Chrome 实机：journey 手势、滚动帧率；不进阶段门禁——单人/本机环境下此项估不出 2d） | — | 人工清单 | （发布窗口内）（✅ 清单已立 §10.8，机器侧发布前项全部完成；实机验收归发布窗口） |
...
**P1b 收口记录 + B0→B1 容差判读（2026-09-22，✅ PASS）**：
- **B1 正式捕获**：270 × 2 双跑，确定性 **270/270 严格零 diff**——B1 成为 P2-P5 全部视觉工作的回归锚点。
- **容差判读三支柱**：① **构造性证明**——几何变更在编辑时即有界：间距归一 violations=0（|Δ|≤6px，宏观留白 10 处保留）、圆角五档映射位移 ≤4px 且全部枚举在案、字阶 ±1px ×79 处；② **统计面**——270 对 median 5.2% / p90 34.8% / max 39.9%；③ **签名抽查**（source-800 差异图目视）——"文本重影 + 整版平移"特征（页头双线、间距阶梯的累积重排级联），无溢出/断行/叠压破坏。**结论：全部差异可归因枚举化预期变更（圆角档位/阴影两级/玻璃两级/间距归一/字阶归档），判读 PASS。**
...
**回滚预案**：全程每阶段独立 commit（P0→P5c 共 20+ 个），`git revert <sha>` 单阶段整体回滚；`src/ui-fixes.test.ts` **26 条绊线**守护全部已修事实，任何回退在测试层先红。CSS 与 className 改动未混入任何逻辑 commit（§0 承诺兑现）。
...
- **P1a 零 diff 核销（270/270 严格零）**：L1/L2 token 并入顶部正式 token 段（兑现段落排序约定）+ 散落 hex **17→0**（阴影墨色→`--shade-soft/deep`、mask 机械色→`--mask-opaque`、12 处 theme-dot 色板→scoped `--dot-a/b` token；值逐字不变）。验证矩阵 270 张对 B0 **tol 0 全零**——连 2 张 AA 常客也零（字形微抖是间歇性的，豁免规则保留）。提交 `5650a66`。
- **P1a.5 对比度定案（L1 变更，本计划唯一成本敏感依赖已锁定）**：扫描器 `scripts/ui-contrast.mjs`（解析真实 token + color-mix 求值 + WCAG + 混合比自动搜索）出具 `docs/ui-contrast.json`。**实测破线**：74% 时四浅色主题最差组合 **classic/text3 on surface3 = 4.11:1**（承载 44×11px + 44×12px meta）；搜索曲线 77%→4.49（差 0.01）、**78%→4.62 达标**。**定案 `--text-3` 74%→78%**（最小达标值，留 0.12 余量）。**代价记录**：B0 的 text-3 像素随之变化 → **B0 立即刷新重拍**（在 P1b 前完成正是本计划排序设计的意图——晚做则 P1b/P2/P3 全矩阵重拍）。
